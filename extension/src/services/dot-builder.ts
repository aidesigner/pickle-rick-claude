...
function isCodergenPhase(
  phase: PhaseSpecInterface,
  phases: PhaseSpecInterface[],
  workspace: 'isolated' | undefined,
): boolean {
  if (phase.goalGate) return false;
  if (phase.securityScan) return false;
  if (phase.specFirst) return false;
  if (!phase.allowedPaths || phase.allowedPaths.length === 0) return false;
  if (phase.allowedPaths.every(p => p.startsWith('docs'))) return false;
  ...
  if (independent.length >= 2) return false;
  return true;
}
...
function hasCrossBranchRetry(phases: PhaseSpecInterface[]): boolean {
  const independent = phases.filter(p => !p.dependsOn || p.dependsOn.length === 0);
  ...
  for (const phase of independent) {
    ...
    for (const otherId of indIds) {
      if (otherId !== thisId && phase.retryTarget.includes(otherId)) return true;
    }
  }
  return false;
}
