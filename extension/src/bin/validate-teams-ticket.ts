...
const USAGE =
  'Usage: node validate-teams-ticket.js --ticket-path <dir> [--role <implementation|review>]';

const VALID_ROLES: readonly WorkerRole[] = ['implementation', 'review'] as const;

function isValidRole(value: string): value is WorkerRole {
...
export function parseArgs(argv: readonly string[]): ParsedArgs {
  ...
  let role: WorkerRole = 'implementation';
...
export async function main(argv: readonly string[] = process.argv.slice(2)): Promise<void> {
  ...
  console.error(
    `Ticket ${ticketPath} is missing required ${role} lifecycle artifacts (need one of each prefix): ${missing.join(', ')}`
  );
  process.exit(1);
}
