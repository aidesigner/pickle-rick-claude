...
import {
  ...
  type MetricSnapshot,
  type RunContext,
} from '../microverse-runner.js';
import type {
  MicroverseSessionState,
  State,
} from '../../types/index.js';

type IterationRunOutcomeInput = Parameters<typeof handleIterationOutcome>[3];
const stateManager = new StateManager();

function makeMicroverseState(): MicroverseSessionState {
  return {
    status: 'iterating',
...
function makeRunnerState(sessionDir: string, workingDir: string): State {
  return {
    active: true,
...
function makeContext(sessionDir: string, statePath: string, currentRunnerState: State): RunContext {
  return {
    sessionDir,
...
function makeBaseline(): MetricSnapshot {
...
function notificationLogPath(homeDir: string): string {
...
function stubSpawnSync(): ReturnType<typeof _deps.spawnSync> {
  return {
    pid: 0,
    output: [null, '', ''],
    stdout: '',
    ...
    status: 0,
    signal: null,
  } as unknown as ReturnType<typeof _deps.spawnSync>;
}
...
async function runWorkerErrorScenario(opts: {
  ...
  try {
    if (opts.notifyEnv === undefined) delete process.env.PICKLE_NOTIFY_ON_ERROR;
    else process.env.PICKLE_NOTIFY_ON_ERROR = opts.notifyEnv;
    ...
    }) as typeof _deps.spawnSync;
    _deps.displayMacNotification = (() => {
      notificationCalls++;
    }) as typeof _deps.displayMacNotification;
    ...
    if (previousNotify === undefined) delete process.env.PICKLE_NOTIFY_ON_ERROR;
    else process.env.PICKLE_NOTIFY_ON_ERROR = previousNotify;
