...
import { printMinimalPanel, Style, formatTime, getExtensionRoot, getDataRoot, buildHandoffSummary, sleep, writeStateFile, markTicketDone, markTicketSkipped, collectTickets, parseTicketFrontmatter, runCmd, safeErrorMessage, ensureMonitorWindow, displayMacNotification, type TicketInfo } from '../services/pickle-utils.js';
...
export function validateAutoTicketCompletion(
  ...
  if (!info) return { action: 'leave', reason: 'malformed_or_missing_ticket_frontmatter' };
  if (isTerminalTicketStatus(info.status)) return { action: 'leave', reason: 'ticket_already_terminal' };
