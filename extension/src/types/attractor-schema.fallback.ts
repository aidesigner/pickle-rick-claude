...
export const ATTRACTOR_SCHEMA_FALLBACK = {
  ...
  node:  Record<string, AttrDef>;
  graph: Record<string, AttrDef>;
  edge:  Record<string, AttrDef>;
};

export type FallbackSchema = typeof ATTRACTOR_SCHEMA_FALLBACK;

/** Flat list of all attribute definitions across all scopes. */
export const ALL_ATTRS: readonly AttrDef[] = [
...
export function lookupAttr(name: string, scope: AttrScope): AttrDef | undefined {
  const bucket = ATTRACTOR_SCHEMA_FALLBACK[scope] as Record<string, AttrDef>;
  return bucket[name];
}
