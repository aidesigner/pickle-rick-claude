...
function bashWritesProtectedConfig(command: string): string | null {
  ...
  for (let i = 0; i < tokens.length - 1; i++) {
    if (tokens[i] === '>' || tokens[i] === '>>') {
      ...
      if (isProtectedConfigToken(dest)) return path.basename(dest);
    }
  }
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (!REDIRECT_DEST_COMMANDS.has(cmdToken) && !CONFIG_INPLACE_WRITE_COMMANDS.has(cmdToken)) continue;
    for (let j = i + 1; j < tokens.length; j++) {
      const arg = tokens[j];
      if (arg.startsWith('-')) continue;
      if (isProtectedConfigToken(arg)) return path.basename(arg);
    }
  }
...
function detectBashStateWriteTarget(command: string): { matched: string; isSettings: boolean } | null {
  ...
  for (let i = 0; i < tokens.length; i++) {
    const cmdToken = path.basename(tokens[i]);
    if (!REDIRECT_DEST_COMMANDS.has(cmdToken)) continue;
    for (let j = i + 1; j < tokens.length; j++) {
      const arg = tokens[j];
      if (arg.startsWith('-')) continue;
