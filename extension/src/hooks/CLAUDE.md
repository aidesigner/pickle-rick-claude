...
- **Chained-command segmentation**: the worker-forbidden-op detectors (`detectProhibitedGitVerb`, `isBashInvokingInstallSh`, `extractNodeTestPath`) evaluate every `splitShellSegments` segment, not just the leading token — `cd sub && git reset` cannot bypass them. See trap-door entry for `handlers/config-protection.ts` (chained-command detection).
