 ...
 */
function isDroidEnvelopeLine(line) {
    if (!line.trim())
        return false;
    try {
        const parsed = JSON.parse(line);
 ...
 */
function extractDroidAssistantContent(lines) {
    const parts = [];
    for (const line of lines) {
        if (!line.trim())
            continue;
...
function isSetupJsInvocation(command) {
...
function extractBashFromAnthropicToolUse(obj) {
    if (obj.type !== 'tool_use' || obj.name !== 'Bash')
        return null;
    return readStringField(obj.input, 'command');
}
function extractBashFromCodexInvocation(obj) {
    if (obj.name !== 'Bash')
        return null;
...
function extractBashCommandFromJson(obj) {
...
function observeBashFromAnthropicAssistant(obj) {
    ...
    for (const block of content) {
        if (!block || typeof block !== 'object' || Array.isArray(block))
            continue;
        const result = observeBashFromObject(block);
        if (result !== null)
            return result;
    }
    return null;
}
function observeStreamJsonLine(trimmed) {
    let parsed;
    try {
...
function observeCodexBlockLine(trimmed) {
    try {
...
export function observeCodexToolCallStream(streamLine, mode) {
    try {
        ...
        if (!trimmed)
            return null;
        if (mode === 'stream-json')
            return observeStreamJsonLine(trimmed);
        if (mode === 'codex-block')
            return observeCodexBlockLine(trimmed);
        return null;
    }
    catch {
        return null;
...
export function detectOutputFormat(output) {
    ...
    if (lines.some(isDroidEnvelopeLine))
        return 'droid';
    return 'plain-text';
}
