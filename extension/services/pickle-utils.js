...
export const TICKET_TIER_BUDGETS = {
    ...
    medium: { max_iterations: 30, worker_timeout_seconds: 40 * 60 },
...
export function hasCompletionCommit(args) {
    ...
    if (inferred)
        return { sha: inferred.sha, source: 'inferred' };
    return { sha: null, source: 'absent' };
}
/**
...
function recreateMissingWatcherPane(sessionDir, sessionName, watcher, spawnSyncFn, logTag) {
    ...
    appendWatcherRestartLog(sessionDir, `${logTag} WARN: pane ${watcher.pane} missing; recreated via ${splitSpec.target}`);
