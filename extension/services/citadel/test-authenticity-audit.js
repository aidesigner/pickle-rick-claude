...
const VACUOUS_TYPE_PRESENCE_RE = /Object\.keys\([^()]*\)\s*\.\s*toContain\(\s*['"]([A-Z][A-Za-z0-9_]*)['"]\s*\)/g;
const LOCAL_FN_CLASS_RE = /^\s*(?:export\s+)?(?:async\s+)?(?:function|class)\s+([A-Za-z_$][\w$]*)/gm;
const NAMED_IMPORT_RE = /import\s*(?:type\s+)?\{([^}]*)\}\s*from\s*['"]([^'"]+)['"]/g;
export function findVacuousTypePresence(content, file) {
    const findings = [];
...
export function collectLocalDeclarations(content) {
    const names = new Set();
...
export function collectImports(content) {
    const names = new Set();
    const modules = [];
...
export function findInlineCopies(specContent, specFile, siblingExports) {
    const findings = [];
    ...
    for (const [, exports] of siblingExports) {
        for (const name of declared) {
            if (seen.has(name) || imported.has(name) || !exports.has(name))
                continue;
            seen.add(name);
            findings.push({
                ...
                severity: 'Medium',
                file: specFile,
                message: `Test declares '${name}', a sibling export it imports the module for but never imports — `
...
const EXPORT_NAME_RE = /^\s*export\s+(?:async\s+)?(?:function|class|const|let|var)\s+([A-Za-z_$][\w$]*)/gm;
function readSiblingExports(specAbsPath, moduleSpecifier) {
...
export function auditTestAuthenticity(diff) {
    const findings = [];
    for (const changed of diff.changedFiles) {
        if (changed.status === 'D' || changed.kind !== 'test')
            continue;
        ...
        for (const moduleSpecifier of collectImports(content).modules) {
            ...
            if (exports && exports.size > 0)
                siblingExports.set(moduleSpecifier, exports);
        }
