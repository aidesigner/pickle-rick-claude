...
test('each TEST_FILES file is wired into npm test', () => {
  const failures = [];
  const npmTest = PACKAGE_JSON.scripts?.test ?? '';
  for (const filename of TEST_FILES) {
    const filePath = path.join(__dirname, filename);
    try {
    ...
    if (!npmTest.includes(`tests/${filename}`)) {
      failures.push(`${filename}: absent from package.json npm test script`);
    }
  }
