 ...
 */
export function resolvePointerLine(state, microverse) {
    ...
    if (MICROVERSE_STEPS.has(step)) {
        ...
        const conv = microverse.convergence;
...
async function main() {
    ...
    while (true) {
        let stateSnap = {};
        try {
            stateSnap = sm.read(statePath);
        }
        catch { /* session dir unreadable — keep polling */ }
        const microverseData = readRecoverableJsonObject(microversePath);
