...
function insertCompletionCommitField(content, sha) {
    ...
    if (!fm)
        return null;
    if (/^completion_commit:\s*.+$/m.test(fm.body))
        return null;
 ...
 */
function findRecentCommitForTicket(workingDir, ticketId) {
    ...
    const out = execFileSync('git', ['-C', workingDir, 'log', '--grep', ticketId, '--format=%H', '-n', '10', 'HEAD'], { timeout: 5000, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] });
...
export function inspectPhantomDoneTicketFile(filePath, sessionDir, workingDir, priorStatus = 'Todo') {
    ...
    if (gitFailureReason)
        result.gitFailureReason = gitFailureReason;
    return result;
}
...
async function runMuxRunnerMain() {
    ...
    const emitBackfillEvent = (ticketId, commit, ts) => {
        ...
        process.stderr.write(`phantom-Done backfilled for ticket ${ticketId} with commit ${shortSha} (work was done, completion_commit field was missing)\n`);
        try {
            writeActivityEntry(statePath, {
