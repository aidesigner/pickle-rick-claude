...
import { getHeadBranch, updateTicketFrontmatter, isWorkingTreeDirty, listWorkingTreeDirtyPaths, archiveBeforeDestructive, ArchiveAbortError } from '../services/git-utils.js';
...
function countSilentDeathRespawns(statePath, ticketId) {
    try {
        ...
        if (!s || !Array.isArray(s.recovery_attempts))
            return 0;
        return s.recovery_attempts.filter((a) => a && a.strategy === SILENT_DEATH_RESPAWN_STRATEGY && a.outcome === 'success' && a.ticket === ticketId).length;
    }
    catch {
        return 0;
    }
...
function appendSilentDeathAttempt(statePath, attempt) {
    try {
        sm.update(statePath, (s) => {
            if (!Array.isArray(s.recovery_attempts))
                s.recovery_attempts = [];
