...
function validateSessionDir(sessionRoot) {
...
function refuseLiveRunner(statePath) {
    ...
    if (pid !== null && isProcessAlive(pid)) {
        process.stderr.write(`lock-scope: refusing to run — pipeline-runner.js PID ${pid} is still alive.\n` +
...
function patchPipelineJson(pipelinePath, mode, scopeBase) {
    const raw = readRecoverableJsonObject(pipelinePath);
...
function patchStateJson(statePath) {
    const sm = new StateManager();
...
function patchPipelineStatusJson(sessionRoot, pipelinePath) {
