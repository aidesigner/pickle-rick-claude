...
function buildWorkerPrompt(roleId, prdContent, outputFile, workingDir) {
...
function spawnWorker(roleId, prompt, refinementDir, extensionRoot, timeout, workingDir, onComplete) {
    ...
    return new Promise((resolve, reject) => {
        ...
        proc.on('close', () => {
            ...
            logStream.on('finish', () => {
                ...
                const result = { roleId, success, logPath };
                onComplete(result);
                resolve(result);
            });
        });
...
async function main() {
    ...
    if (!prdPath || !sessionDir || prdPath.startsWith('--') || sessionDir.startsWith('--')) {
        console.error(`${Style.RED}❌ Usage: node spawn-refinement-team.js --prd <path> --session-dir <dir> [--timeout <sec>]${Style.RESET}`);
        process.exit(1);
    }
    ...
    const interval = setInterval(() => {
        ...
        process.stdout.write(`\r   ${Style.CYAN}${spinChar}${Style.RESET} ${statusParts} ${Style.DIM}[${formatTime(elapsed)}]${Style.RESET}\x1b[K`);
        spinIdx++;
    }, 200);
    let results;
    try {
    ...
    printMinimalPanel('Refinement Team Complete', Object.fromEntries(results.map((r) => [r.roleId, r.success ? '✅ analysis written' : '❌ failed — check log'])), allSuccess ? 'GREEN' : 'YELLOW', '🥒');
