...
**Autonomy directive (operator, 2026-05-30):** the babysitter (`prds/babysitter.md`) is authorized to drain the ENTIRE master plan with zero operator interaction — including the full release cycle (`git push` + `gh release create`), gated only on a green release gate + clean tree. Nothing in this plan should require operator interaction. The sole permitted residue is work gated on an **external event** (not operator approval): #25 R-CSI / B-CSI forensics need a real concurrent-session incident to analyze — watch-only, skipped by the drain.
...
|---|---|---|---|
| 25 | R-CSI | Concurrent claude-session destructive-command interference (3 SIGINT incidents/36h) — DATA LOSS class | `p1-concurrent-claude-session-interference-with-running-pipelines.md`. **Watch-only — external-event-gated (NOT operator-interaction):** Phase 1 forensics need a real concurrent-session incident to analyze. Skipped by the babysitter drain; surfaces when the next incident occurs. |
...
| 6 | **B-CSI** | WATCH-ONLY | R-CSI Phase 1+2 | External-event-gated (NOT operator-interaction): needs a real concurrent-session incident to scope/analyze. Skipped by the babysitter drain; re-activates on the next incident. |
| 7 | **B-CCDC** | DEFERRED | R-CCDC citadel detection-coverage successor | Operator-deferred by choice (maybe-later) — out of drain scope until re-activated. Not a halt point. |
