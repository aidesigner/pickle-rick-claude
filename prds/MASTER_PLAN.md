...
| Active pipeline | **B-E2E** (drain row 4b, P3, #19 R-MMTR-6 residual) — `prds/p2-bug-fix-bundle-b-e2e-mmtr6.md`, launching 2026-06-01. Build the missing max-turns-relaunch E2E test (R-MMTR-6A fixture → 6B harness → 6C 3-scenario test → 6D CI gate). B-WEDGE v1.89.5 mooted the original fan-out AC; launches without refinement. Schema-neutral (PATCH → v1.89.6). All P1+P2 bug bundles shipped (1.89.0–1.89.5). |
...
| 4b | **B-E2E** | P3 | #19 R-MMTR-6 | `prds/p2-bug-fix-bundle-b-e2e-mmtr6.md` (build max-turns-relaunch E2E test: 6A fixture/6B harness/6C 3-scenario test/6D CI gate; schema-neutral). Source: `p1-mmtr-6-decompose-e2e-into-sub-tickets.md`. B-WEDGE mooted original fan-out AC. **LAUNCHING 2026-06-01.** | ~5 |
