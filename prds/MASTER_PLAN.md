...
**Last updated**: 2026-05-05 mid-day (live forensic during run #5 babysit found **deploy-parity gap**: all 5 hot files in `~/.claude/pickle-rick/extension/` are from `May 3 10:41:42` — predates entire bundle. Cap-split fix `6be334b1` is committed but NOT deployed. R-XBL-1..9 instrumentation events being silently dropped by deployed state-manager. See new "🚨 Live forensic during run #5" section. Slot 1q severity reassessed P2→P1 candidate.) Earlier mid-morning: run #5 wall-clock near-miss → operator reset `start_time_epoch` and filed slot 1t to default-off `max_time_minutes`. Slot 1u filed after observing 154 manager stop-hook turns burned on one 27min worker wait. Next-bundle prep complete: drafted slots 1o/1p/1q/1u + bundle wrapper `prds/p1-bug-fix-bundle-2026-05-05.md` (queue #0-next, ships v1.71.0) composing 9 source PRDs into 53-67 atomic tickets; awaits run #5 close + v1.70.0 tag. Earlier (early hours): P1 bug-fix bundle 2026-05-04 RUNNING on session `2026-05-04-f416c6cc` via fresh tmux `pipeline-fresh-f416c6cc`, claude-only Path X, max_iterations=500 GLOBAL with **R-CNAR-1 part 2 cap-split fix** (commit `6be334b1`) live. Section A keystones complete: **R-XBL-1 / -2 / -2b / -3 + R-CNAR-1 + R-CNAR-1-part-2 SHIPPED** (commits `7793a11 9437b0c 817e73c a3641e3 616f474 95f2c37 6be334b1`). Bundle worker also shipped Section B's R-DTS-1 + R-BUNDLE-CLEANUP + several Section A residuals (R-XBL-5/-6/-7/-7b/-8/-9, AC-XBL-08, AC-EVENT-PAYLOAD-01) — see commits `cd35ae82 6f1a5486 e5d64089 50c43b9c 8c692f2e 7ef0c041 a2690794 044a8d42 7d81aad6 ee2ae138`. **Status: 14 Done / 48 Todo.** Run #4 (10-iter cap, claude-only) shipped 9 tickets cleanly before tripping the cap conflation bug; the cap-split fix (R-CNAR-1 part 2) addresses that root cause so run #5 can run end-to-end without manual relaunches. Run #2 forensics already filed slots 1o (manager/worker backend split), 1p (codex-spark completion_commit reliability), 1q (install.sh types-index deploy gap). Slot 1r filed 2026-05-05 from `pipeline-2026-05-04-8aecd4c7`: anatomy-park `validateWorkerConvergenceHistory` fails when metric_type='none'. **89 unpushed commits**, 0 tagged. v1.66.0 still GitHub-Latest with poison content; bundle closer ships v1.70.0. Slot 1l codex-spark wiring SHIPPED locally; tagged with v1.70.0.)

**Bootstrap for new sessions**: read `CONTEXT_2026-05-04.md` first.
...
**Run #5 LAUNCHED 2026-05-05 01:28 local** on tmux `pipeline-fresh-f416c6cc` with claude-only Path X + cap-split fix (`6be334b1`) + max_iterations=500 GLOBAL. With 14 Done / 48 Todo, the bundle should ride this one launch to completion (no more cap-trip relaunches needed; per-ticket budgets still enforced independently).

While run #5 runs:

1. **Babysit** via `tmux attach -t pipeline-fresh-f416c6cc` (Ctrl+B 1 = monitor, Ctrl+B 0 = runner, Ctrl+B D = detach). Watch for `worker_spawn_backend_resolved` events with `backend=claude source=state` (R-XBL-2 SoT honored). Watch for any `iteration_cap_exhausted` exit_reason — if it fires, check whether `budgetIter` (per-ticket) or `state.iteration` (global) tripped first.
2. **Slot 1f (R-ICP-7) push pending** — R-ICP-1..6 already-shipped on session `2026-05-03-7d9ee8cc` (commits `434e33d a7ed2a9 2de4c24`); R-ICP-7 regression test still owed. Defer until bundle closes.
3. **Draft slot 1o / 1p / 1q PRDs** — proposed in run #2 forensics, not yet drafted. After bundle closes, file these and queue for next bundle:
   - 1o `p1-worker-backend-split-from-manager.md` — adds optional `state.worker_backend` field; manager=claude, worker=codex-spark hybrid (saves cost on bulk worker spawns while keeping orchestration reliable).
   - 1p `p2-codex-spark-worker-completion-commit-contract-violation.md` — workers commit but skip `completion_commit:` frontmatter; phantom-Done watcher wrongly reverts them.
   - 1q `p2-install-sh-types-index-stale-on-fast-reinstall.md` — post-rsync md5-parity probe + `rm types/index.js && tsc` force-rebuild guard.
4. **Slot 1r filed 2026-05-05** — `prds/anatomy-park-judge-unreachable-on-worker-convergence.md` (10-LOC fix; pickle phase shipped 21/21 + anatomy-park converged but `validateWorkerConvergenceHistory` returned `judge_unreachable` → szechuan skipped). Bundle behind 1o/1p/1q.

**After run #5 closes (closer ships v1.70.0)**, file the next bundle: **`prds/p1-bug-fix-bundle-2026-05-05.md`** (drafted 2026-05-05, slot **#0-next**) composes Sections A–H over slots 1o, 1p, 1q, 1r/1s, 1t, 1n, 1m, 1d. Estimated 48-61 atomic tickets after refinement. Closer ships **v1.71.0**. Section ordering: C (install.sh parity gate) FIRST, A (worker_backend) before B (completion_commit), H (test flakes) BEFORE closer. Open question: ship 1q as a v1.70.1 hotfix BEFORE this bundle, since it gates the bundle's own integrity? Refinement Cycle 3 decides.
...
| 1e | [`prds/p2-refined-tickets-trip-readiness-contract-resolver.md`](p2-refined-tickets-trip-readiness-contract-resolver.md) **NEW P2** | **Composed into bundle 2026-05-04 Section D** (R-RTRC-1..6 + R-RTRC-7 NEW annotation schema). | Land via bundle slot #0 |
...
| 1g | [`prds/p1-deploy-typescript-symlink-and-cap-no-auto-resume.md`](p1-deploy-typescript-symlink-and-cap-no-auto-resume.md) **P1 — KEYSTONES SHIPPED** | **R-DTS-1 + R-CNAR-1 + R-CNAR-1 part 2 SHIPPED.** R-DTS-1 regression test landed `6f1a5486`. R-CNAR-1 tier budgets landed `9437b0c` (medium=30, large=60, xlarge dropped). R-CNAR-1 part 2 cap-split landed `6be334b1` (global vs per-ticket caps now independent — was the actual root cause of "pipeline keeps dying after 10 iters" symptom). R-DTS-2/-3 + R-CNAR-2..6 still queued via bundle slot #0. | Bundle slot #0 ships remainder. |
...
| 1k | [`prds/p3-monitor-watcher-continuous-auto-respawn.md`](p3-monitor-watcher-continuous-auto-respawn.md) **NEW P3** | **Composed into bundle 2026-05-04 Section E** despite original "defer" — included because it shares pane-respawn + watchdog fixture surface with Section A diagnostics. AC-MWR-03 split into 03a (3 file-tail watchers parametrized) + 03b (refinement-watcher manifest-poll separate path; zero `FEED TERMINATED` matches). RESPAWN_WATCHDOG_INTERVAL_MS rename ships first commit to avoid collision with MONITOR_STDOUT_WATCHDOG_MS (R7). | Land via bundle slot #0 |
...
| 1m | [`prds/p3-pipeline-runner-dirty-tree-guard-blocks-self-cleanup.md`](p3-pipeline-runner-dirty-tree-guard-blocks-self-cleanup.md) **NEW P3** (filed 2026-05-04 PM during bundle launch) | **Draft** — pipeline-runner exits FATAL on `bundle/ac-dr-02.json` (test-debris regenerated by `verify-recapture-fired` on every test run with random temp-dir paths). Bundle's own R-BUNDLE-CLEANUP ticket cannot fix because the guard fires at iteration 0. 4 R-PDT requirements: stop writing ephemeral data into tracked file (verify-recapture-fired stable digest); guard ignore-list consults `.gitignore` + `extension/.pipeline-runner-dirty-allowed.json`; explicit dirty-file list in stderr; trap-door entry. Workaround: `git checkout -- bundle/ac-dr-02.json`. | File next bundle after v1.70.0 ships |
| 1n | [`prds/p2-stop-hook-blocks-launcher-of-tmux-bundle-via-orphan-session.md`](p2-stop-hook-blocks-launcher-of-tmux-bundle-via-orphan-session.md) **NEW P2** (filed 2026-05-04 PM during bundle launch) | **Draft** — three compounding bugs surfaced: (1) stop-hook default-fallthrough at `stop-hook.ts:198-203` lacks `state.tmux_mode === true` check; launcher conversations of tmux-owned loops are blocked indefinitely; (2) `recoverStaleActiveFlag` does not bridge `state.pid=null` AND `current_sessions.json` mapped-PID-dead — orphan sessions stay active forever; (3) mapped-session filter selects orphans over live-same-cwd fallback. User-visible: "🥒 Pickle Rick Loop Active (Iteration 0 of 100)" feedback was reading orphan session `b20c7a0a` not live bundle session `f416c6cc`. 4 R-SHB requirements + 5 ACs + workaround applied (orphan demoted, current_sessions.json repointed). | File next bundle after v1.70.0 ships |
| 1o | [`prds/p1-worker-backend-split-from-manager.md`](p1-worker-backend-split-from-manager.md) **NEW P1 — manager/worker backend split** (drafted 2026-05-05) | **Drafted** — 8 R-WBS requirements + 8 ACs. Adds optional `state.worker_backend` field; spawn-morty/microverse-runner consult it first, fall back to `state.backend`. Manager continues reading `state.backend`. PICKLE_REFINEMENT_LOCK=1 still wins. New `worker_backend_resolved` activity event. Symmetrical with existing `state.codex_model` per-session override. Removes the F1 hallucination class architecturally (no longer depends on R-XBL-3 catching the flip after the fact). | Land via bundle 2026-05-05 Section A |
| 1p | [`prds/p2-codex-spark-worker-completion-commit-contract-violation.md`](p2-codex-spark-worker-completion-commit-contract-violation.md) **NEW P2** (drafted 2026-05-05) | **Drafted** — 4 R-CCC requirements + 7 ACs. Three-layer mitigation: (a) ACK-token directive in worker prompt; (b) post-commit auto-fill helper for `completion_commit:` frontmatter; (c) phantom-Done watcher cross-checks git log and writes `completion_commit_inferred:` instead of reverting. Mitigation B is the keystone; A and C are belt-and-suspenders. Slot 1o sidesteps this for manager-narrated commits but worker-direct commit class persists. | Land via bundle 2026-05-05 Section B |
...
| 1r+1s | [`prds/anatomy-park-judge-unreachable-on-worker-convergence.md`](anatomy-park-judge-unreachable-on-worker-convergence.md) **NEW P1 — TWO-SECTION BUNDLE: judge-unreachable defects in `microverse-runner.js`** (filed 2026-05-05) | **Draft** — bundled because both bugs live in the same file and share the assumption that the judge can be silently bypassed without breaking convergence semantics. **Section 1 (slot 1r)** — `validateWorkerConvergenceHistory` (lines 447–474) returns `judge_unreachable` after worker writes `converged: true` because metric_type=`'none'` never populates `convergence.history`. Live repro on `pipeline-2026-05-04-8aecd4c7`: 21/21 tickets shipped, citadel green, anatomy-park converged (`consecutive_clean=3`, 0 trap doors) — runner threw it away with exit 1. Fix: skip guard when `currentMv.metric?.type === 'none'`. **Section 2 (slot 1s)** — `measureLlmMetric` (lines 793–833) ETIMEDOUT silently treats two consecutive judge-CLI timeouts as `stall` → `converged`. Live repro on `2026-05-05-af779f40`: baseline timed out (`defaulting to 0`) AND iteration 2 timed out twice — runner converged on the worker's TASK_NOTES.md self-report alone, no judge score ever produced. Fix: distinguish `judge_timeout` from `stall`; exponential backoff `[10,30,60]s` with 3 attempts; baseline-fail exits with `baseline_unmeasurable`; CLI-presence smoke probe at session start. **Combined**: ~30 LOC + 12 ACs (6 per section). | File next bundle after v1.70.0 ships |
| 1t | [`prds/p2-remove-pipeline-wall-clock-time-cap.md`](p2-remove-pipeline-wall-clock-time-cap.md) **NEW P2 — remove wall-clock cap default** (filed 2026-05-05) | **Draft** — `state.max_time_minutes` enforcement leaks across `--resume` (start_time_epoch preserved) and bites rate-limit recovery (4h reset window collapses against remaining budget → `exit_reason='limit'` reported as clean-success). Live repro: bundle session `2026-05-04-f416c6cc` run #5 was 500min into a 720min cap at launch — operator had to manually reset `start_time_epoch` to avoid losing 48 unshipped tickets. Iteration caps (`max_iterations`=500 + per-ticket tier ceilings) and per-worker timeouts already bound resource usage; the wall-clock cap is the wrong primitive. The "no cap" branch already exists at every enforcement site (9 sites all check `maxTimeMins > 0`); this PRD only flips the default. 10 R-NTC requirements + 12 AC-NTCs. Supersedes `large-pipeline-time-budget-undersized.md` AC-LPB-07. | File next bundle after v1.70.0 ships |
| 1u | [`prds/p2-manager-stop-hook-nudge-cadence-wastes-turns.md`](p2-manager-stop-hook-nudge-cadence-wastes-turns.md) **NEW P2 — manager stop-hook nudge cadence wastes turns during worker waits** (filed 2026-05-05) | **Draft** — stop-hook nudges manager every ~2s; during long-running large-tier worker waits, manager replies `"Waiting for Monitor signal."` (8 output tokens, 153K cache reads) for 100+ consecutive turns. Live forensic on bundle session `2026-05-04-f416c6cc` run #5 ticket 51d826c9: 154 stop-hook turns in 27min worker wait, 133/154 are degenerate `"Waiting for…"` replies. Burns through `--max-turns 400` budget at ~50 turns/15min, forcing gratuitous mid-worker manager respawns. Two-layer fix: (1) `WAIT_PATTERN_REGEXES` in stop-hook detects degenerate replies; (2) after 3 consecutive matches, switches to event-aware nudge (state.json mtime / worker-PID-liveness / artifact-landing / fallback timer). 6 R-MSCN requirements + 9 AC-MSCN. Sister to slot 1k (monitor pane respawn) and slot 1t (wall-clock cap). | File next bundle after v1.70.0 ships |
