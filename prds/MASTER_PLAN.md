...
17. **Szechuan-sauce LLM judge non-deterministic scoring → false-stall** *(NEW — OPEN, P1)* — Discovered 2026-05-09 PM in active session `2026-05-09-92dbdff2` (operator: `/szechuan-sauce --backend codex packages/api/src/lib/appraisal-pipeline` post-anatomy-park cleanup on `loanlight-api@gregory/1025-appraisal-epic`, scope branch). Operator instinct: "I do think the scoring might have a bug." Confirmed: `metric.type === 'llm'` szechuan-sauce sessions stall on the convergence floor even when commits are demonstrably removing real principle violations every iteration. Score history iter 2→8: `8 → 6 → 5 → 5 → 5 → 5 → 5` with classifications `improved → improved → improved → held → held → held → held`. `stall_counter: 4/5` despite 4 real Observability fixes landing in iters 5–8 (`708a1645d` agentic strategy failure logging, `89d10639b` Nest logger stale-cleanup, `b09312bff` Nest logger OCR preprocessor, `2fbd16489` agentic verbose logging structure). Root cause: `buildJudgePrompt` (`microverse-runner.ts:1145-1186`) gives the LLM judge no stable violation IDs and no diff-aware context — only one-line prior scores (`Iteration N: score=X action=accept — improved/held: X vs Y`); the judge does a fresh code scan each iteration and outputs the count of ~5 most-prominent violations it sees. Inspection of `convergence.history[].metric_value` raw text shows iter 5 listed `[ensemble-voter, cost-constants, console-OCR, console-stale, console-agentic]`, iter 6 listed `[ensemble-voter, cost-constants, extractSalesCompPage god, detectBinaryChoice large, findCompColumnAnchors large]`, iter 8 listed `[ensemble-voter, cost-constants, getLineItems-dup, SectionAnchor-dup, SOURCE_BONUS_MID-noop]` — different violations with same score 5. `compareMetric` (`microverse-state.ts:143-160`) is purely numeric (`current vs previous`); composes badly with non-deterministic LLM judge. Symmetric failure mode to `prds/szechuan-sauce-codex-judge-model-mismatch.md` (false-convergence at 0). Anatomy-park does NOT have this bug because it uses worker-managed convergence with stable-ID `findings_history` per subsystem and `consecutive_clean: 2` per-subsystem (structural, not numerical). Severity P1 — convergence-blocking + misleading the operator + budget-burning over a 50-iteration ceiling. **PRD: `prds/p1-szechuan-sauce-llm-judge-non-deterministic-scoring-false-stalls.md`** (R-SLLJ-1..8). **Fix options:** Option A (durable, ~1wk) — stable violation ledger added to `microverse.json`, judge reuses IDs and emits set diff, `compareMetric` uses set-ops; Option B (cheap, risky) — judge counts entries in worker-maintained `gap_analysis.md` instead of fresh-scanning; Option C (incremental, ~1-2d) — extend judge prompt with prior_violations list + structured `{resolved, new, remaining}` output. Recommend Option C as stop-gap, Option A as durable. Sister to Finding #16 (R-PRJT) and Finding #13 (R-MJCP) — same `microverse-runner.ts` file, same metric-path family, all three add up to "LLM-judge metric path is the most fragile surface in convergence today." Interim mitigation while open: raise `stall_limit` 5 → 15 for LLM-judge szechuan-sauce sessions to defer false-stall.
