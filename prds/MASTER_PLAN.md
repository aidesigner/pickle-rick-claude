...
**2026-06-02 — NO LONGER FULLY DRAINED.** A loanlight-api LOA-955 remediation pipeline (build → review → anatomy-park → szechuan-sauce) surfaced 3 new findings, filed as **B-SMAF** (drain row 15, P2): #91 R-SMAF (scoped microverse + out-of-scope `lint --fix` → dirty-tree abort, recurring), #93 R-APXG (anatomy-park post-convergence exit-gate hang), #92 R-RSBI (resolve-scope base inconsistency). Source: `prds/BUG-REPORT-2026-06-02-scoped-microverse-out-of-scope-autofix-dirty-tree-abort.md`.
...
| 15 | **B-SMAF** (LAUNCHING) | P2 | #91 R-SMAF (+ #93 R-APXG, #92 R-RSBI) | `prds/p2-bug-fix-bundle-b-smaf-scoped-microverse-out-of-scope-autofix.md` (authored 2026-06-02 from the bug report). R-SMAF-1: scope-aware clean-tree precondition in `microverse-runner.ts` (evaluate dirtiness only over `allowed_paths`, mirroring the gate's baseline subtraction) + regression. R-RSBI-2: `resolve-scope.ts` single documented base + de-conflate abort copy. R-APXG-3: bound the anatomy-park post-convergence exit gate (no indefinite hang). Each ticket's research phase confirms the mechanism before editing. Schema-neutral (PATCH unless a new event added → MINOR). | ~4 |
| 16 | **B-HERMES** (verify-first) | P2 | (feature — integration) | `prds/hermes-integration.md`. **Residual-check FIRST** — `hermes` is already wired in `backend-spawn.ts`, so this is likely already-shipped (R-PSAI class). Babysitter: grep HEAD for the PRD's ACs; if all present → close-as-shipped (no launch); if gaps → author a remaining-scope bundle + launch. Promoted from deferred 2026-06-02 per operator. | ~1 verify |
...
| 91 | R-SMAF | Scoped `/anatomy-park`/`/szechuan-sauce` runs let a package-wide `lint --fix` mutate files **outside** `scope.json:allowed_paths`; the runner's clean-tree precondition then aborts (`Working tree is dirty and not a git repository`) and **recurs every gate/iteration** until the unrelated autofix is manually committed. Survives B-APWS (#11, which gated worker `git add`, not lint-autofix of the working tree). **OPEN.** Source: `prds/BUG-REPORT-2026-06-02-scoped-microverse-out-of-scope-autofix-dirty-tree-abort.md`. Drain **B-SMAF** (row 15). |
| 93 | R-APXG | anatomy-park post-convergence "gate before exit" hangs indefinitely after the worker signals convergence (`converged=true`, trap door committed) — session never prints the closing banner, must be killed manually. Likely coupled to R-SMAF (gate can't reach a clean tree). **OPEN.** Source: same report (secondary). Adjacent: `prds/anatomy-park-judge-unreachable-on-worker-convergence.md`. |

### P3

| # | Code | Summary | Notes |
|---|---|---|---|
| 92 | R-RSBI | `resolve-scope.js` base inconsistency: the same `--scope paths:<glob>` resolved against `packages/api` for one template/run but the repo root for another → `SCOPE_EMPTY_PATHS` unless the operator changes the glob prefix. Same `workingDir`-vs-`repoRoot` class as R-CWRR (#88). Plus the abort copy conflates "dirty (out-of-scope)" with "no `.git` found". **OPEN.** Source: `prds/BUG-REPORT-2026-06-02-scoped-microverse-out-of-scope-autofix-dirty-tree-abort.md` (secondary). |
