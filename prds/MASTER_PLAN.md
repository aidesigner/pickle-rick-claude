...
| Item | Value |
|---|---|
| Version (source/deployed) | **v1.91.0** — 2026-06-01 |
| Latest GitHub release | v1.90.1 (v1.81.1..v1.90.1 all tagged) |
| Active pipeline | **none** — B-DWF impl phase ✅ SHIPPED v1.91.0 (R-DWF-2/4/5: dynamic-workflow conversion of refine-prd + council, behind kill-switches; spike R-DWF-1 PASSED). R-DWF-5 spurious validation-flip self-recovered via manager-respawn (mrc=2). Full gate green (fast 5492 / integration 779 / expensive 13). **R-DWF-3 (retire old subprocess path) is soak-gated + R-DWF-6 (docs) depends on it → follow-up bundle B-DWF-2 after a green soak.** 10 releases shipped this session. **Next: B-DSEK (row 14, P3 feature).** |
