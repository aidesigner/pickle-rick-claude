...
| Item | Value |
|---|---|
| Version (source/deployed) | **v1.90.1** — 2026-06-01 |
| Latest GitHub release | v1.90.1 (v1.81.1..v1.90.1 all tagged) |
| Active pipeline | **none** — B-LASP ✅ SHIPPED v1.90.1 (#87 closed). Closer caught the first R-LASP-1 attempt silently disabling validation (root `$ref` stub loaded via naive `JSON.parse`, reddening `log-activity-gate-payload.test.js`); the PRD premise was false (install.sh has no schema logic + excludes `src/`, so the deployed tree had no real schema bytes). Re-scoped: resolver `[src/types-first, root-second]` + install.sh now copies the real schema to the deployed root. **Next: feature epics (B-DWF/B-DSEK).** |
...
| 12 | **B-DWF** (IN PROGRESS) | P2 | (feature — no bug finding) | `prds/p2-dynamic-workflow-conversion-refine-prd-council.md`. Gates B-PNTR + B-ACSG cleared (both shipped). **R-DWF-1 hard spike PASSED 2026-06-01** (verdict PROCEED, all 6 probes PASS — `prds/research/dwf-spike-findings.md`: gate-bin-shell ran headless, state-write firewall holds for workflow agents, 33-thunk batch OK, agents are Claude). Gate cleared; re-launched to drive R-DWF-2 (WS-A refine workflow) + R-DWF-4 (WS-B council), then R-DWF-3 (soak-gated)/5/6. Caveats (non-blocking): allowlist additions = deployment-time operator decision on global settings.json; codex-companion.mjs not deployed = R-DWF-4 scope. Schema-neutral. | ~6 |
