...
| 1h | [`prds/p2-worker-silent-exit-and-ticket-path-drift.md`](p2-worker-silent-exit-and-ticket-path-drift.md) ⭐ **NEW P2** | **Draft** — Surfaced on ticket `ab62807f`: worker exited after research-review APPROVED with NO downstream lifecycle artifacts (no plan/implement/verify/review). `worker_session_<pid>.log` was 0 bytes (silent exit). Validation correctly marked Failed but no diagnostic surfaced. Two RC: RC-1 worker session log 0-byte exits (no flush before exit); RC-2 refinement-team produces tickets with `Files to modify: <path>` where the path doesn't exist at HEAD (e.g., ab62807f cited `services/resolve-state.ts` which doesn't exist; correct location is `state-manager.ts:recoverStaleActiveFlag`). 4 R-WSE + 4 R-RPD requirements. | Fix after current bundle completes; tickets passed RC-2 audit go to retry queue |
