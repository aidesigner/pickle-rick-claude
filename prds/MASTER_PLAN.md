...
| 17 | **B-CBI** (R-CBI) | P2 | (feature — integration) | `prds/p2-cli-backend-integration-pattern.md` — one contract for native-CLI Shape-B backends; instances **grok + kimi** (neither wired in `backend-spawn.ts` — genuinely pending). Follows the B-DSEK backend pattern (type + builder + spawn-site wiring + setup guards + tests). Refinement-check the PRD's ticket decomposition before launch; MINOR (new backends). Promoted from deferred 2026-06-02 per operator. | ~5 |
