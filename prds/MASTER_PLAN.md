...
| **B-HCAG** | 🔴 NEXT (P1, operator-approved 2026-05-18) | R-HCAG (hallucinated-conformance attestation gate) | ~6 + 3 hardening | **MUST SHIP BEFORE B-SJET-2.** PRD `prds/p1-hallucinated-conformance-attestation-gate.md`. Mechanical gate against the hallucinated-acceptance class (Finding #2 successor): `parseConformanceCitations` extracts file/symbol citations from worker conformance text, `verifyCitationsAgainstDiff` grep-validates against `git diff <base>..HEAD`, mux-runner halts with `conformance_citation_drift` if citations cite symbols absent from diff. R-HCAG-4 forbids Done flip without non-null `completion_commit`. R-HCAG-6 e2e-replays f00097e8 hallucinated conformance fixture; halt expected. Ships first so B-SJET-2 + B-SSDF + B-QSRC run under hardened gate. **Adversarial-review-on-this-bundle** explicit in PRD: closer MUST replay f00097e8 fixture before tagging. |
| **B-SJET-2** | 🔴 QUEUED (P1, after B-HCAG) | R-SJET-1/2/3/4/6 residual (Finding #47, unshipped portion) | ~5 + 3 hardening | **HIGH priority — Finding #47 still OPEN; B-SJET partial-shipped without LLM judge fix.** **DO NOT LAUNCH UNTIL B-HCAG SHIPS** — the hallucinated-conformance failure mode that killed f00097e8 must be gated first. Same PRD `prds/p1-szechuan-sauce-judge-etimedout-baseline-measurement.md`. Must land at HEAD: (1) `stdio: ['ignore', 'pipe', 'pipe']` on `measureLlmMetricAttempt` + `probeJudgeCliAvailability` (R-SJET-1); (2) async `execFile` + `Promise.race` against hard timer with `JudgeMeasurementTimeout`/`JudgeMeasurementSpawnFailed` typed errors (R-SJET-2 LLM path); (3) judge env isolation when running nested under Claude Code (R-SJET-3); (4) `judge_backend: 'auto'` settings with codex fallback on typed timeout (R-SJET-4); (6) end-to-end integration test against fake-claude-hang binary (R-SJET-6). Closer-owned verify-then-close against PRD reproducer mandatory. |
