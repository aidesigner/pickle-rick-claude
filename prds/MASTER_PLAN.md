...
**2026-06-02 — bug queue drained; FEATURE EPICS draining.** B-SMAF (#91/#92/#93) shipped v1.92.2 (closer partial-ship babysitter-completed: gate green serialized after a c=2 contamination false-RED; the 3-finding fixes are live). B-HERMES (row 16) closed as ALREADY SHIPPED (hermes fully integrated: Backend type + BACKENDS + builder + 4 state fields + generic dispatch + --teams rejection + hermes-smoke/lifecycle tests). **Now draining feature epics: B-CBI (row 17, grok+kimi) → B-GFR2 (row 18).** Residue: B-DWF-2 ⏸️ shelved + #25 R-CSI external-gated.

## Status

| Item | Value |
|---|---|
| Version (source/deployed) | **v1.92.2** — 2026-06-02 |
| Latest GitHub release | v1.92.2 (v1.81.1..v1.92.2 all tagged) |
| Active pipeline | **NONE — drained.** Last shipped: **B-SMAF** ✅ v1.92.2 (row 15) — scope-aware clean-tree precondition (R-SMAF) + resolve-scope single-base/abort-copy (R-RSBI) + bounded anatomy-park exit gate (R-APXG); closes #91/#92/#93. 13 releases shipped this session (v1.89.0–v1.92.2). Prior residue: B-DWF-2 ⏸️ SHELVED (row 13): its hard-gate soak surfaced a real workflow bug (ManifestSchema `format:date-time` rejected by Workflow-runtime AJV) → **FIXED standalone in `e1c322f2`** (landed on main; improves the opt-in workflow); the post-fix re-soak got all 3 analysts green but stalled on undefined synthesis input paths (soak-harness invoked the workflow without `args` — workflow code is correct). Retiring the safe-default legacy refinement path is low-value + not justified on a soak that can't cleanly validate → shelved, legacy path retained (zero regression). The soak's findings were converted to shipped hardening: **v1.92.1** tags the date-time fix + a fail-fast arg-validation guard in `refine-analyze.js`. **12 releases shipped this session (v1.89.0–v1.92.1); 2 stale findings closed; 1 workflow bug found+fixed via soak.** Residue = NON-actionable: B-DWF-2 (needs a reliable soak harness that passes `args` before re-attempt) + #25 R-CSI (external-event-gated forensics). |
...
| 18 | **B-GFR2** | P3 | (refactor) | `prds/god-functions-remediation-phase-2.md` — 27 god-function carve-outs. LARGE refactor, broad blast radius (run gitnexus_impact per carve-out; expect churn). Drains LAST (P3, after the P2 features). Likely benefits from `/pickle-refine-prd` to decompose the 27 carve-outs into atomic tickets before launch. Promoted from deferred 2026-06-02 per operator. | ~27 |
