...
| 11b | **B-LASP** | P3 | #87 R-LASP | `prds/p2-bug-fix-bundle-b-lasp-log-activity-schema-resolution.md` (authored 2026-06-01). Deployed `bin/log-activity.js:44` resolves the activity schema via `../src/types/…` — a path `install.sh` never creates (schema is deployed to the extension root) → `Failed to load activity schema: ENOENT`, fail-open. Confirmed live 2026-06-01. Fix: deploy-target-first + in-repo-fallback resolution in `log-activity.ts` + deploy-parity test. Schema-neutral, PATCH (→1.90.1). **LAUNCHING 2026-06-01.** | ~2 |
