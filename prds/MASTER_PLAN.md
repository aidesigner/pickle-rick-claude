...
| 19 | **B-MFW** | P2 | R-MFW (MCP forwarding) | `prds/p2-bug-fix-bundle-b-mfw-mcp-forwarding-to-workers.md` (authored 2026-06-02 from `prds/p2-mcp-forwarding-to-workers.md`). Promoted from the deferred-integration bucket on operator backend-depth demand — workers/managers/refinement on ALL six shipped backends are MCP-blind (Linear-blindness; LOA-789 worked example). Was gated on R-MFW+R-CBI both shipping → **unblocked by B-CBI v1.93.0**. 7 tickets (R-MFW-1 settings → -2 claude `--mcp-config` Option A → -3 codex `-c` injection Option B/D-stub → -4 setup-time snapshot Option D → -5 refinement forwarding → -6 `worker_mcp_config_resolved` event → -7 real-subprocess integration test + AC-8 trap door) + closer. Judges stay sandboxed (INV-SANDBOX-JUDGE); `--ignore-user-config` intact (INV-IGNORE-USER-CONFIG). Schema-neutral → **MINOR** (new settings fields + new event + forwarding surface). Unblocks citadel NEW-T6 + R-PTG-7. | ~8 |
