...
**Last updated**: 2026-05-04 PM (P1 bug-fix bundle 2026-05-04 IN FLIGHT on session `2026-05-04-f416c6cc` — codex-spark backend, 62 atomic tickets refined via 3-cycle team, R-XBL-1 first ticket Done at iteration ~2, R-XBL-2 currently mid-implement on `extension/src/services/backend-spawn.ts`. Refined PRD bakes in Cycle-3 critical fixes: R-XBL-4 DROPPED (already-shipped), R-DTS-1 → REGRESSION-TEST-ONLY, AC-DTS-02 nonexistent --help replaced, AC-MWR-03 split 03a/03b, AC-BUNDLE-03 NAME COLLISION renamed to AC-PIPELINE-PHASES-01, xlarge tier dropped (5-invariant blast radius), Section C dogfooding strike, launch.sh references → mux-runner.js, R-BUNDLE-DISPO-1 disposition table prevents R-TAQ-2 hallucinated-premise self-halt. Two new bug PRDs surfaced during launch: P3 dirty-tree-guard chicken-and-egg + P2 stop-hook blocks launcher conversation via orphan-session shadow. **77 unpushed commits**, 0 tagged. v1.66.0 still GitHub-Latest with poison content; bundle closer ships v1.70.0. Slot 1l codex-spark wiring SHIPPED locally; tagged with v1.70.0.)
...
- **P1 bug-fix bundle 2026-05-04 launched** — session `2026-05-04-f416c6cc`, codex-spark backend, 62 atomic tickets (56 impl + 1 wire + 4 harden + 1 closer). Refinement complete (3 cycles × 3 analysts on claude, ~30 min wall, 9/9 succeeded, 6 ac_shape_smells resolved via parametrization). First ticket R-XBL-1 (`52277694`) Done at iteration 2; R-XBL-2 currently mid-implement. Bundle PRD `prds/p1-bug-fix-bundle-2026-05-04.md` committed at `862381f`; refined version (61K, +31K) committed at `1f3c530`. Closer (order 750) bumps v1.70.0 + invokes `closer-release-gate.sh` (R-CLOSER-1) — finally evicts poisoned v1.66.0 from GitHub-Latest.
...
Bundle is in flight on `2026-05-04-f416c6cc`. Babysit via `tmux attach -t pipeline-f416c6cc` or `/pickle-status`. Order of attack post-bundle:

1. **Bundle's wiring + 4 hardening + closer must complete** before any release tag. Closer (`R-CLOSER-1` script) is idempotent under partial gh-API failures; tag is LAST step.
2. **After closer ships v1.70.0**: file the next bundle from the residual queue:
   - 1d test-flakes (council-publish + scope-resolver) — pre-existing flakes from v1.69.0 that survived this bundle
   - P3 dirty-tree-guard (R-PDT-1..4) + P2 stop-hook (R-SHB-1..4) from today's surfaced bugs
   - 1k monitor watchdog (deferred; subsumed by Section E of THIS bundle's R-MWR but residuals possible)
   - hermes-integration + multi-repo-task-state-drift + god-functions-remediation-phase-2 (queue slots 5-7)
3. **Bundle execution constraints** carried over: codex-spark recovery CUJ (R20 in refined PRD); operator presence for first 30min (R-CNAR-6 smoke gate); foreground-only auto-resume (R21).

## 🔔 Active Queue (priority order)

| # | PRD | Status | Next action |
|---|---|---|---|
| **0** | [`prds/p1-bug-fix-bundle-2026-05-04.md`](p1-bug-fix-bundle-2026-05-04.md) ⭐ **P1 BUG-FIX BUNDLE — IN FLIGHT 2026-05-04 PM** | **62 atomic tickets** (56 impl + 1 wire + 4 harden + 1 closer) on session `2026-05-04-f416c6cc`, codex-spark backend, max-iter=∞ max-time=∞. Refinement: 3 cycles × 3 analysts on claude (claude-only by design), 9/9 succeeded, 6 ac_shape_smells resolved via parametrization, R-BUNDLE-DISPO-1 disposition table (52 IMPLEMENT + 1 DROP + 1 REGRESSION-TEST-ONLY) prevents recursive bootstrap failure. R-XBL-1 (first ticket, `52277694`) Done at iteration 2; R-XBL-2 mid-implement on `extension/src/services/backend-spawn.ts`. Bundle composes 1e+1g+1h-WSE+1i+1j+1k from prior queue plus NEW: R-BUNDLE-1 (single bootstrap flag), R-BUNDLE-2 (baseline snapshot), R-BUNDLE-DISPO-1 (disposition table), R-CLOSER-1 (idempotent release-gate script). Bootstrap flags applied: `state.flags.bundle_bootstrap_mode="2026-05-04-v1.70.0"` + `state.flags.skip_readiness_reason`. Cycle-3 critical refinements baked in (R-XBL-4 DROP, R-DTS-1 RTO, AC-DTS-02 replaced, AC-MWR-03 split, AC-BUNDLE-03→AC-PIPELINE-PHASES-01 rename, xlarge dropped, etc). | Babysit; closer ships v1.70.0 with `gh release create --latest` evicting v1.66.0. |
...
| 1j | [`prds/p1-worker-spawns-codex-despite-claude-backend.md`](p1-worker-spawns-codex-despite-claude-backend.md) **NEW P1 — cross-backend leak** | **Composed into bundle 2026-05-04 Section A** as the integrity-critical FIRST section. R-XBL-4 DROPPED (already-shipped at codex-manager-relaunch.ts:69 + mux-runner.ts:2078-2086, :3206-3212 per codebase-analyst grep), replaced by AC-XBL-08 regression test only. R-XBL-2 enumeration corrected to 4 real spawn sites (spawn-gate-remediator excluded; gets R-XBL-2b inheritance audit). NEW R-XBL-7b (reproduces actual session 2026-05-03-7d9ee8cc bug under PICKLE_REFINEMENT_LOCK=1) + R-XBL-9 (refinement-claude prompts reference new event schemas). | Land via bundle slot #0 |
