...
## 🆕 2026-05-05 finding — slot 1r filed

`pipeline-2026-05-04-8aecd4c7` (claude backend, `INCOME_EXPANSION_FIX_PRD.md` over `loanlight-api-income-expansion`): 21/21 atomic tickets shipped over 173m of pickle phase (B1-B4 / M1-M7 / m1-m9 — every defect from a 3-agent review of `feat/income-expansion`). Citadel green (5 medium / 1 low, all benign). Anatomy-park converged across 2 iterations (`consecutive_clean=3`, 0 trap doors). **Then `validateWorkerConvergenceHistory` returned `judge_unreachable` and the runner exited 1 → szechuan-sauce skipped.** This is a sibling of the v1.63.0 finalizer-history-crash fix — same architectural assumption (`mvState.convergence.history` is populated under worker-mode), different code path (a guard added later in microverse-runner.js:447–474). Filed as slot 1r. ~10-LOC fix.
...
**After run #5 closes (closer ships v1.70.0)**, file the next bundle from residuals:

- 1d test-flakes (council-publish + scope-resolver) — pre-existing, not addressed by this bundle
...
- 1k monitor watchdog (residuals from R-MWR-* if any survive run #5)
- 1o + 1p + 1q (drafts described above)
- 1r anatomy-park judge-unreachable (pickle convergence false-fail)
...
| 1o | (proposed) `prds/p1-worker-backend-split-from-manager.md` **NEW P1 — manager/worker backend split** (filed 2026-05-04 evening from run #2 forensics) | **Not yet drafted** — F1 finding: codex-spark manager hallucinated backend flips during bundle run #2. Architectural fix: introduce optional `state.worker_backend` field. spawn-morty consults `worker_backend` first, falls back to `state.backend`. PICKLE_REFINEMENT_LOCK=1 still wins. Manager continues reading `state.backend` (claude/sonnet for reliable orchestration). Workers run on `state.worker_backend` (codex-spark for cheap+parallel implementation). ~15 LOC + state-field invariant + test. Symmetrical with existing `state.codex_model` per-session override. Removes the F1 hallucination class entirely; doesn't depend on R-XBL-3 catching the flip after the fact. | Block bundle slot #0 relaunch on this; ship before run #3 |
| 1p | (proposed) `prds/p2-codex-spark-worker-completion-commit-contract-violation.md` **NEW P2** (filed 2026-05-04 evening) | **Not yet drafted** — F2 finding: codex-spark workers commit code to git but skip writing `completion_commit: <sha>` into ticket frontmatter. Phantom-Done watcher (correctly per contract) reverts the ticket to Todo, manager re-spawns, fast-loops with no progress, circuit breaker trips. Run #2 lost `8224fc7f / 160e8816 / 4d7c4cfa` to this even though git had the real commits. Mitigations: (a) strengthen the worker prompt's "MUST write completion_commit" constraint (currently a `**MUST** ... NEVER flip status: Done before the commit exists` paragraph in spawn-morty.ts:436 — codex-spark ignores it); (b) post-commit wrapper that auto-fills frontmatter from `git log -1 --format=%H -- <ticket-path>`; (c) phantom-Done watcher cross-checks git log when frontmatter lacks the field. Slot 1o (claude manager) sidesteps this for the manager-narrated commits but the worker-direct commit class persists. | Bundle behind slot 1o |
| 1q | (proposed) `prds/p2-install-sh-types-index-stale-on-fast-reinstall.md` **NEW P2** (filed 2026-05-04 evening) | **Not yet drafted** — F3 finding: first install.sh after R-CNAR-1 + R-XBL-2 left deployed `extension/types/index.js` md5-mismatched against source — deployed copy missing 8 events (`worker_spawn_backend_resolved`, `paused_session_orphan_demoted`, etc.). State-manager rejected events as unknown (`WARN: ignoring unknown activity event`) for the entire 28-min run. md5 parity probe at end of install.sh + fail-loud on mismatch would have surfaced this in 2 seconds. Suspect cause: `npx tsc` did not actually recompile types/index.js before install.sh's rsync ran (tsc incremental cache + source-file mtime same-second?). Fix: install.sh runs `rm extension/types/index.js && npx tsc` BEFORE rsync, then post-rsync runs `md5 source deployed | uniq -c` to assert parity on the 5 most-trafficked compiled files. Re-running install.sh manually fixed the immediate gap. | Bundle behind slot 1o |
| 1r | [`prds/anatomy-park-judge-unreachable-on-worker-convergence.md`](anatomy-park-judge-unreachable-on-worker-convergence.md) **NEW P1 — sibling of v1.63.0 finalizer-history fix** (filed 2026-05-05 from `pipeline-2026-05-04-8aecd4c7`) | **Draft** — `microverse-runner.js:validateWorkerConvergenceHistory` (lines 447–474) returns `judge_unreachable` after worker writes `converged: true` because metric_type=`'none'` never populates `convergence.history`. Same architectural assumption as the shipped finalizer crash, different code path (a guard added later). Anatomy-park exits 1 → pipeline-runner halts → szechuan-sauce skipped. Live repro on `pipeline-2026-05-04-8aecd4c7` over `INCOME_EXPANSION_FIX_PRD.md`: 21/21 tickets shipped, citadel green, anatomy-park converged across 2 iterations (`consecutive_clean=3`), 0 trap doors — runner threw it away. Fix: skip the guard when `currentMv.metric?.type === 'none'`. ~10 LOC + 6 ACs. | File next bundle after v1.70.0 ships |
...
| `anatomy-park-judge-unreachable-on-worker-convergence.md` | **Draft (P1)** filed 2026-05-05 — sibling of v1.63.0 finalizer fix | 6 ACs; ~10-LOC fix in `microverse-runner.js:handleWorkerManagedIteration`; reproduces deterministically on any `metric_type='none'` worker-managed phase |
