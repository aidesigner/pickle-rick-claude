...
| Active pipeline | **B-LSOF** (drain row 10, P3, #37e R-PIWG-5) — `prds/p2-bug-fix-bundle-b-lsof-launch-git-probe.md`, launching 2026-06-01. Advisory launch-time concurrent-git-access probe (extract fail-OPEN helper + setup.ts wire-in + event). Schema-neutral (PATCH → v1.89.7). 7 bundles shipped this session (1.89.0–1.89.6); B-MONITOR closed as already-shipped. Next: R-PSAI (P3), then feature epics (B-DWF/B-DSEK). |
