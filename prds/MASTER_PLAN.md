...
| 1f | [`prds/p1-iteration-cap-and-phantom-done-handshake.md`](p1-iteration-cap-and-phantom-done-handshake.md) ⭐ **NEW P1** | **Draft** — Three coupled bugs in mux-runner ↔ codex ↔ pipeline-runner handshake: (A) `setup.js --resume` doesn't honor persisted `max_iterations`, defaults to 15 vs the original 500; (B) mux-runner exits **code 0** on cap-hit-without-EPIC_COMPLETED, pipeline-runner advances as if phase succeeded; (C) codex flips ticket `status: Done` speculatively, mux-runner phantom-Done detector runs only at iteration boundaries so flips slip through. Surfaced when reliability-bundle session pickle phase exited "successfully" with 13/38 tickets done; pipeline-runner advanced to anatomy-park; downstream phases ran on incomplete output. 7 R-ICP requirements; recommend fix order B→C→A. | Fix this round (agent team) so pipeline can resume cleanly |
