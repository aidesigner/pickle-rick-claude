...
| 4 | **B-PNTR** | P1 | #77 R-PNTR-DEPS | RE-SCOPE first (see #77 — target is a code path, not the deleted prompt file); then `prds/p2-remove-non-tmux-pickle-loop.md` | ~1 re-scope |
...
| 77 | R-PNTR-DEPS | B-PNTR R-PNTR-1 (`d586b545`) wrongly deleted load-bearing `.claude/commands/pickle.md` (mux-runner prompt composition) → `[FATAL] pickle.md not found` on next dispatch. Restored `40f22573`. **B-PNTR scope must be RE-DERIVED before any further R-PNTR work:** the actual "bare non-tmux loop" target is a code path in `mux-runner.ts` / `spawn-morty.ts`, NOT the prompt-template file. Sized ~1 (re-scope). **B-PNTR.** |
...
- **Integrations:** `hermes-integration.md` (P2 ready), `deepseek-integration.md` (P3), `openrouter-multi-provider-workers.md` (P3)
