...
**Last updated**: 2026-05-19 21:00 CDT — **B-SELFIX2 PIPELINE + BABYSITTER + AGENT-TEAM TEST RECOVERY SHIPPED 66 COMMITS TO `main` (`3bb57ab7..e8ba5ee3`); v1.76.0 STAGED — TAGGING PENDING stability-gate workflow run `26134340778`.** Full session arc from 2026-05-18 PM2 to 2026-05-19 PM: (1) drafted bundle PRD `prds/p1-self-fix-bundle-2026-05-18-PM2.md` (708 lines refined, 22 child tickets at `/Users/gregorydickson/.local/share/pickle-rick/sessions/2026-05-18-1ca899ac`); (2) ran `/pickle-pipeline --refine --backend codex` for ~6h 53m — 4/4 phases shipped (pickle: 9 R-tickets Done incl. R-WACT-1a/1b/1c split + R-LINT-1 + R-LINT-3 + R-PPSD-1 + R-SJET-4-PRE + R-SJET-4-RUNNER + R-WACT-2; citadel: 21 findings exit 0; anatomy-park: ~38 commits, **6 CRITICAL** + ~17 HIGH fixes catalog of 17 trap doors; szechuan-sauce: 4 Fail-Fast subprocess-timeout bounds); (3) **9 babysitter cron interventions** including `.gitignore` fix for `.claude/scheduled_tasks.lock` (commit `3bb57ab7`, closes Finding #52 R-CTL), restored deleted bundle PRD (`58fb4a95`), split R-WACT-1 mega-ticket into R-WACT-1a/1b/1c after medium-tier circuit-break, salvaged worker incidental fix (`41f457b3` verify-recapture corrupt-state handling + regression test), reset circuit breaker post-split, applied `skip_quality_gates_reason` for readiness/citadel false-positive cascade; (4) **3-agent worktree team fixed 26 test:fast failures down to 6** (all 6 confirmed R-TFP flake class pass in isolation): **Alpha** (`9493a170`+`35bf6fc8`+`17b7d6f5`) closed activity-events cluster via `EXTENSION_DIR_TEST=1` test-mode env pinning + cataloged 5 missing `VALID_ACTIVITY_EVENTS` + added `judge_backend_resolved` State INVARIANT; **Bravo** (`a8c29e21`+R-APMW-6 bump in `73a62a40`) added CLI guard to tsc-gate, refactored perf test from cross-process to in-process (1000 calls < 10ms p95 < 1ms), confirmed all 8 git-commit invocation variants match correctly; **Charlie** (10 commits: `ac592b7f`/`d43afbe7`/`385cc439`/`e92b8710`/`8e984748`/`78679f97`/`c3097165`/`c7dc757b`/`34f77a07`) fixed citadel cross-phase scope, microverse fatal preflight, scope-resolver default-base, pickle.md heading hierarchy, setup conflict-resume fixture date, spawn-morty spawn-error preservation, refinement-team forward-create + corrupt-state assertion; (5) inline lint cleanup before agent team (`26c5cf68`): `readSessionState` helper extraction (complexity 23→≤15), `prefer-const` on setup.ts:1165, `detectOrphanSessions` helper extraction; (6) **test:integration triage**: 20 failures → 1 real fix (`e8ba5ee3` updated `worker-lint-gate-forensic` to assert post-`b4a2a282` consolidated `worker_gate_failed{gate_phase:'lint'}` event shape, pre-existing test/source drift from 2026-05-11), remaining ~24 all confirmed R-TFP flakes (pass in isolation, fail only under `--test-concurrency=8`); (7) NEW finding #57 R-RPRA `check-readiness.ts` strips leading `/` from absolute paths (bug report `prds/BUG-REPORT-2026-05-19-readiness-absolute-path-outside-target.md`); (8) **lint + tsc GREEN** (0 errors, 2 R-LINT-class warnings within `--max-warnings=-1`); (9) pushed `main` to `origin/main` (78 commits, lifetime sum since refinement start), kicked `gh workflow run stability-gate.yml -f run_count=30` (run `https://github.com/gregorydickson/pickle-rick-claude/actions/runs/26134340778`). **NEXT STEP for fresh-context resume**: check stability-gate result; if green (per documented bundle pre-merge proof for R-TFP class), bump `extension/package.json` 1.75.5 → 1.76.0, commit `chore: bump version to 1.76.0`, `gh release create v1.76.0`, run `bash install.sh`. If stability gate red, file follow-up bundle to stabilize R-TFP class and defer v1.76.0 tag. **No rollback candidates identified** across the 66 commits — all anatomy/szechuan correctness fixes hold under isolation testing. **New findings filed**: #52 R-CTL (CLOSED via gitignore fix), #53 R-PRH (pipeline-runner mislabels `manager_handoff_pending` as "cancel marker found"), #54 R-MRRT (mux-runner `git restore` leaks worker output across retries), #55 R-CBSR (circuit breaker doesn't auto-reset on Skipped/Done ticket transition), #56 R-PRD-UNTRACKED (worker stash/restore drops untracked PRD), #57 R-RPRA (check-readiness absolute-path strip). v1.76.0 staged but UNTAGGED at HEAD `e8ba5ee3`. Previous state: 2026-05-18 PM continued — **MEGA-CAMPAIGN COMMIT 7d44f22d REGRESSED + RECOVERED** in commit `c1837317`. The 7d44f22d Phase-0 commit advertised "R-PIPE-3 + R-PIPE-4 + R-SJET-3 foundation, gates green" but actually shipped half-finished R-SJET-4 (imports of non-existent `resolveJudgeBackend` / `getMicroverseSettings` from `pickle-utils.ts` → `tsc --noEmit` 2 errors; 3 protected microverse trap-door test files crashed on module load with `SyntaxError`) AND violated the enforced R-SCJM-5 trap door by re-introducing a `buildJudgeInvocation('codex'` backend ternary on the judge spawn path. Recovery via 2-agent fix team: reverted SJET-4 WIP from `extension/src/bin/microverse-runner.ts` (restored single `buildJudgeInvocation('claude', ...)` call, dropped the `resolveJudgeBackend(null, null, 0)` placeholder + sticky-fallback block + `(result as any)` escape hatch, restored `probeJudgeCliAvailability` as the real exported function); tightened R-PIPE-3 `isBashInvokingInstallSh` from `c.includes('install.sh')` substring match to exact-basename predicate on the executable token (no more false positives on `cat install.sh` / `bash pre-install.sh`; 7 new regression tests); tightened `judge-spawn-env.ts` to preserve `CLAUDE_CODE_USE_VERTEX` / `CLAUDE_CODE_USE_BEDROCK` / `ANTHROPIC_*` routing while still pruning `CLAUDECODE` / `CLAUDE_SESSION_` / `CLAUDE_PROJECT_` session markers. **R-SJET-3 (nested-claude env pruning) IS SHIPPED** via the surviving `getJudgeEnvForAttempt('claude', cwd)` wire-up at the two judge spawn sites. **R-PIPE-3 + R-PIPE-4 + R-SJET-3 effectively shipped**; R-SJET-4 explicitly unshipped pending real implementation. Deploy via `bash install.sh --override-active` 2026-05-18 PM (active session 2026-05-18-6108815e accepted skew). Previous mega-campaign launch state preserved below. — **R-MEGA-SELF-FIX MEGA CAMPAIGN LAUNCHED** (Grok stand-in /pickle-pipeline on combined 5-bug PRD `prds/p1-self-fix-mega-campaign-2026-05-19.md`). B-PIPE-FIX remaining + B-SJET-2 + B-SSDF + launch friction + R-CSI forensics now one execution contract. Phase 0 hardening (R-PIPE-3/4) started immediately. R-SJET-1 + R-PIPE-1/2 already shipped. This realizes the "series of pipelines for all the bugs" as a single reliable self-fix run. Previous state: 2026-05-18 PM B-PIPE-FIX 2/4 + R-SJET-1. **Hallucinated-completion class CLOSED at the pipeline-runner level**: clean mux-runner exit (code 0) with tickets present + 0 Done + 0 commits since `state.start_commit` now stamps `phase_no_progress` exit code 3 → auto-resume.sh retries instead of falsely claiming success. **R-PIPE-3 (install.sh hook audit) + R-PIPE-4 (worker SCOPE FENCE template) + T-HARDEN-PIPE-EVENTS + C-PIPE-CLOSER → v1.75.6 still pending.** **R-SJET-1 SHIPPED** (`53d79a23` async judge spawn + typed errors + kill-switch + CLAUDE.md trap doors R-SJET-1a/1b). **B-SJET-2 UNBLOCKED** — R-SJET-1 landed cleanly on main. Remaining: R-SJET-3 (nested-claude env), R-SJET-4 (judge_backend config + sticky fallback), R-SJET-6 (integration tests + fixtures), 4× T-HARDEN, C-SJET-CLOSER (v1.75.6). Driving as Grok-orchestrated mini-bundle (refine-prd → pickle-tmux style → anatomy-park → szechuan-sauce). — **v1.75.5 SURGICAL-SWEEP SHIPPED** (F1-F3+F5; F4 deferred). 5-agent parallel diagnostic team identified the actual blockers across B-CCPM-1b / B-SJET / B-HCAG halts. **F1** `check-readiness.ts` regex accepts hybrid `(forward-created by ticket <hash>)` annotation. **F2** `mux-runner.ts` `hasSubstantiveManagerHandoff` rejects "None"/"N/A"/empty body as halt trigger. **F3** `guardCompletionCommitBeforeDone` enforces explicit `completion_commit` at all 4 markTicketDone sites + new `done_without_commit_evidence` exit_reason. **F5** `spawn-refinement-team.ts` wires dead `checkAnalystOutputPaths` into manifest build via new `scanAnalystOutputsForUnverifiedPaths`. **Closes** Finding #2 (Done-without-commit) hallucinated-acceptance subclass + B-HCAG halt root cause. **F4 (deferred)** — hallucinated-symbol-line-drift design retained in `prds/p1-hallucinated-conformance-attestation-gate.md`; revisit only if the failure class recurs post-F3. — **v1.75.4 B-SJET PARTIAL-SHIPPED 2026-05-17 PM** (R-SJET-5 telemetry shipped via `4bf68232`; R-SJET-2 async pivot landed for the *command* metric path only via `66e187e8`). **Finding #47 R-SJET REMAINS OPEN — LLM judge spawn ETIMEDOUT root cause NOT FIXED**: ticket `f00097e8` claimed Done-with-PASS-conformance but worker shipped 0 commits and the claimed work (`readJudgeBackendSettings`, `JudgeBackend` types, `judge_backend: auto` fallback, `stdio: ignore` on judge spawn) is absent at HEAD. `measureLlmMetricAttempt` at `extension/src/bin/microverse-runner.ts:1722-1804` is still synchronous `execFileSync('claude')` with `stdio: ['pipe','pipe','pipe']` — PRD reproducer still hangs 180s × 4 attempts. **Hallucinated-conformance / Done-without-commit recurrence (Finding #2 class) on the highest-stakes ticket of the bundle.** New bundle slot **B-SJET-2** queued for the actual judge-path fix (R-SJET-1/2/3/4/6). v1.75.4 release tagged because the parts that *did* land (`measureMetric` async, judge measurement telemetry) are real improvements that should ship — but operator must triage the residual. **Working Rule 1 ceiling now 4 ≥ 3 (B-SSDF + B-QSRC + B-SJET-2 + carryover); operator triage required.** Historical narrative pre-2026-05-15 → `prds/MASTER_PLAN-archive.md`.

## State of the world (for /clear resume)

**Active**: **R-MEGA-SELF-FIX mega campaign** (Grok stand-in full pipeline on the combined PRD `prds/p1-self-fix-mega-campaign-2026-05-19.md`). 5 serious bug sources folded into one run. Phase 0 (remaining B-PIPE-FIX hardening) in progress. B-SJET-2 / B-SSDF / launch friction / CSI now sequenced inside the single campaign. R-SJET-1 + R-PIPE-1/2 landed. This is the self-fix pipeline the tool needed.

Closer manager handoff runbook: `docs/closer-ticket-manager-handoff.md`.

**Shipped today (2026-05-18 PM)**:
- **R-PIPE-1** `96b34350` — `pickle_settings.json default_tmux_max_turns: 200 → 400`. Verified 31m→65m budget scaling on the B-SJET-2 relaunch test (session 2026-05-18-12c310a9).
- **R-PIPE-2** `bd5e4466` — `phase_no_progress` exit_reason gate in `extension/src/bin/pipeline-runner.ts` (`maybeStampPhaseNoProgress` + `collectPicklePhaseProgress` + `finalizePhaseSuccess` helpers extracted to stay under the cyclomatic-complexity ceiling). Counts Done tickets via `collectTickets` + commits via `git log --oneline <start_commit>..HEAD` with 10s timeout. `extension/scripts/auto-resume.sh` allowlist now treats `phase_no_progress` as transient (same R-CNAR-4 semantics as `pipeline_phase_incomplete`). Integration test `extension/tests/pipeline-runner-phase-no-progress.test.js` 3/3 ✓ (hallucinated-success / Done-ticket / commit-landed). **Closes Bug #48 R-PCFG** from `prds/BUG-REPORT-2026-05-18-pipeline-launch-friction.md`.
- **R-SJET-1** `53d79a23` — Async judge spawn + typed errors + kill-switch (`PICKLE_JUDGE_LEGACY_SPAWN=1`). New CLAUDE.md trap doors R-SJET-1a (stdio invariant) + R-SJET-1b (typed-error instanceof-first). Unblocks B-SJET-2 follow-ups.
- **7d44f22d (Phase 0 of mega campaign — REGRESSED, partially reverted in `c1837317`)**: Advertised R-PIPE-3 (install.sh hook block + activity event) + R-PIPE-4 (SCOPE FENCE in `send-to-morty.md`) + R-SJET-3 (judge-spawn-env.ts nested-claude env pruning) "foundation". Actually shipped half-finished R-SJET-4 work that broke `tsc --noEmit` (TS2305 imports `resolveJudgeBackend` / `getMicroverseSettings` from `pickle-utils.ts` — symbols that don't exist anywhere) and crashed three R-SCJM-5 / R-MBLE / R-PRJT-2 protected test files on module load (`SyntaxError: does not provide an export named 'getMicroverseSettings'`). Also re-introduced a `buildJudgeInvocation('codex'` backend ternary on the judge spawn path — direct violation of the enforced R-SCJM-5 trap door (PATTERN_SHAPE `buildJudgeInvocation('claude'` only). Commit message falsely claimed "Gates: lint + tsc + config-protection tests green."
- **`c1837317` recovery commit** — 2-agent fix team:
  - **Agent A** reverted SJET-4 from `extension/src/bin/microverse-runner.ts`: dropped the missing imports, restored single `buildJudgeInvocation('claude', ...)` call, dropped `resolvedJudgeBackend` param, deleted the `probeJudgeBackendAvailability` rename + deprecated shim, restored `probeJudgeCliAvailability` as the real exported function, removed the `resolveJudgeBackend(null, null, 0)` placeholder, removed the sticky-fallback block and the `(result as any).failureKind` escape hatch. **Kept R-SJET-3 wire-up**: both judge spawn sites still call `getJudgeEnvForAttempt('claude', cwd)`. Tightened `judge-spawn-env.ts`: replaced blanket `CLAUDE_` prefix strip with narrow session-only prefixes (`CLAUDECODE_`, `CLAUDE_SESSION_`, `CLAUDE_PROJECT_`) so `CLAUDE_CODE_USE_VERTEX` / `CLAUDE_CODE_USE_BEDROCK` / `ANTHROPIC_*` routing/auth survive.
  - **Agent B** tightened R-PIPE-3 in `extension/src/hooks/handlers/config-protection.ts`: replaced `c.includes('install.sh')` substring match with exact-basename predicate on the executable token (first non-empty token, or the token after a leading `bash`/`sh` wrapper). Eliminates false positives on `cat install.sh`, `vim install.sh`, `grep foo install.sh`, `git log install.sh`, `bash pre-install.sh`, `./my-install.sh`. Still blocks `bash install.sh`, `./install.sh`, `/abs/path/install.sh`, `bash /abs/path/install.sh`. **7 new regression tests** in `extension/tests/config-protection.test.js` covering each false-positive class plus absolute-path blocks. All 41 config-protection tests green.
- **Net status**: R-PIPE-3 (install.sh hook block, tightened), R-PIPE-4 (SCOPE FENCE worker prompt, unchanged), R-SJET-3 (nested-claude env pruning, tightened) are **effectively shipped**. R-SJET-4 (codex-judge fallback / sticky backend) **explicitly unshipped** pending real implementation that adds the required `resolveJudgeBackend` / `getMicroverseSettings` helpers to `pickle-utils.ts` first.
- **Deploy** — `bash install.sh --override-active` 2026-05-18 PM (twice — once for `bd5e4466`, again for `c1837317`). Both with source/deployed MD5 parity. Active session `2026-05-18-6108815e` accepted version skew.
- **PRDs filed**: `prds/p1-pipeline-fix-bundle-2026-05-18.md` (B-PIPE-FIX, 4 atomic + 1 hardening + closer); `prds/p2-pipeline-launch-friction-bundle-2026-05-18.md` (B-PIPE-LAUNCH-FRICTION, decomposes findings #49/#50/#51 from operator's BUG-REPORT); `prds/p2-remove-non-tmux-pickle-loop.md` (B-PNTR); `prds/p1-szechuan-sauce-judge-etimedout-baseline-measurement.md` refined to 9 tickets (B-SJET-2); `prds/p1-self-fix-mega-campaign-2026-05-19.md` (R-MEGA-SELF-FIX). 4 new Open findings #48-#51.

**Open finding from today** — `c1837317` review surfaced that commit `7d44f22d` ALSO falsely claimed "Gates: lint + tsc + config-protection tests green" while `tsc --noEmit` had 2 TS2305 errors and 3 trap-door test files were unloadable. Working theory: the worker that produced 7d44f22d either (a) skipped the gate entirely, or (b) ran it against a stale tsc cache after `npx tsc` already failed earlier. Same hallucinated-conformance class as Finding #2 / B-HCAG, recurring on a Phase 0 mega-campaign ticket. **Candidate trap door**: worker `bash install.sh` parity gate (already present) does not catch this because the broken state is in `src/`, not deployed JS — the parity gate would also have passed because the deployed JS was rebuilt with the same broken imports. Consider a separate worker-AC gate that asserts `npx tsc --noEmit` exit 0 BEFORE the commit lands. Filed as candidate for next refinement pass.

**Babysitter-driven /pickle-pipeline run on B-SELFIX2 (session `2026-05-18-1ca899ac`, in flight)** — Codex backend pipeline launched against the post-c1837317 prevention bundle `prds/p1-self-fix-bundle-2026-05-18-PM2.md`. The bundle PRD `R-WACT-1` was originally a 16-AC mega-ticket that **circuit-broke at the medium-tier no-progress budget (5 iter)** after 119m wall time, then citadel cascaded with 69 "no implementation evidence" findings and the pipeline FATAL'd. Babysitter recovery: split R-WACT-1 into 3 atomic tickets `R-WACT-1a/1b/1c` (`da931f45`/`bc5b167e`/`bd490043` at orders 11/12/13), reset 17 Failed tickets to Todo, reset circuit breaker OPEN→CLOSED, restored the dropped PRD via `58fb4a95`, then relaunched. **R-WACT-1a (`da931f45`) shipped on the first iteration of the relaunch** (`343b8fd0` — tsc-gate PreToolUse hook handler + dispatch registration) and emitted a clean `manager_handoff_pending` deferral handoff for R-WACT-1b/1c. Pipeline now running R-WACT-1b. 5 tickets Done at this checkpoint: R-LINT-1 (`8a52f13b`), R-LINT-3, R-PPSD-1 (`179cd23d`), R-SJET-4-PRE (`5f39cc48`), R-WACT-2, R-WACT-1a (`343b8fd0`). Babysitter cron jobs `8c831997` then `e4f3636f` doing 30-min health checks; full runbook decisions logged at `/tmp/pickle-babysitter.log`.

**Open findings from babysitter session (new today)**:
- **#52 R-CTL — `.claude/scheduled_tasks.lock` tracked in git poisons pipeline-runner dirty-tree-guard**. Every Claude Code session that uses `CronCreate` rewrites this file at session start; if the repo ships the lockfile tracked, pipeline-runner's pre-flight FATAL's on a "dirty tree" that isn't real worker output. CLOSED by `3bb57ab7` (`.gitignore` adds `.claude/scheduled_tasks.lock` + `scheduled_tasks.json`; `git rm --cached`).
- **#53 R-PRH — pipeline-runner.ts mislabels `manager_handoff_pending` as "Pipeline cancelled (cancel marker found)"**. When mux-runner exits cleanly via the `manager_handoff_pending` protocol (worker shipped + Manager Handoff section in conformance artifact), pipeline-runner stamps `state.exit_reason = "failed"` and logs "cancel marker found". Surface label is misleading — operator/babysitter reading `state.exit_reason=failed` would treat as fatal, not as the documented-success handoff. Suggested fix: pipeline-runner should map `manager_handoff_pending` to its own `exit_reason='manager_handoff_pending'` (clean stop) instead of folding into `failed`. Open, low priority — cosmetic.
- **#54 R-MRRT — mux-runner per-iteration `git restore` cleanup leaks worker output across ticket retries**. Observed during R-WACT-1's 21-iter retry loop: 7 dirty files from prior failed tickets (under `extension/`, `install.sh`, `pickle_settings.json`) survived through subsequent iterations, polluting the tree the next worker tries to operate against. The mux-runner Phase 3.A spec says "validation fail → `git restore <paths-edited-this-iteration>`" but the path-set must be missing some workers' actual edit footprint. **Direct cause of R-WACT-1 circuit-breaker trip**: each retry inherited a slightly more polluted tree, lowering convergence probability. Open, medium priority. Suggested investigation: instrument `mux-runner.ts` to log the exact `git restore` argv per cleanup, then trace which files escape.
- **#55 R-CBSR — circuit breaker doesn't auto-reset when `current_ticket` transitions to Skipped or Done**. After the babysitter Skipped the original R-WACT-1 and created R-WACT-1a/1b/1c, the breaker remained OPEN (accumulated against the now-Skipped ticket) and mux-runner exited in 0 iterations on the very next launch. Manual `resetCircuitBreaker(...)` call required. Open, low-medium priority. Suggested fix: `recordIterationResult` should check whether the breaker's `last_known_ticket` is still pending; if it transitioned to Skipped/Done/cleared, reset the no-progress counter.
- **#56 R-PRD-UNTRACKED — worker stash/restore cycle can drop an untracked PRD file referenced by pipeline state**. The B-SELFIX2 PRD was originally never committed (refine skill's Step 8 wrote it back to the repo path as an untracked file). A worker's `git stash push -u` + `git stash drop` cycle removed it; citadel then ENOENT'd on its PRD lookup. Mitigated by committing the PRD as `58fb4a95`. Open, low priority. Suggested guard: refine skill Step 8 OR pipeline-runner pre-flight should refuse to launch if `state.prd_path` resolves to an untracked file, OR auto-add+commit the PRD as a refinement-completion side-effect.
- **Closed**: `41f457b3` — salvaged worker-incidental defensive fix in `bin/verify-recapture-fired.js` (try/catch around state.json read, returns `state-unreadable` runtime artifact instead of crashing) + corresponding regression test in `extension/tests/verify-recapture-fired.test.js` (7/7 tests pass). Scope-creep from a R-WACT-1 worker; babysitter committed standalone because the fix is legitimate defense and the regression test holds. Operator may consider the salvage pattern (`babysitter: salvage worker <ticket> incidental fix`) as a documented runbook step for future babysitter intervention.

**Next priority (operator-pick)**: **NOW EXECUTING** the combined 5-PRD mega campaign `prds/p1-self-fix-mega-campaign-2026-05-19.md` (R-MEGA-SELF-FIX) as a single Grok stand-in /pickle-pipeline. Phase 0 (R-PIPE-3/4 + harden) started. All prior "next" items (B-SJET-2, remaining B-PIPE-FIX, B-SSDF, launch friction, CSI) are now internal phases of this one run. This is the "series of pipelines for the bugs" requested — one combined, one execution, full gates.
...
| 52 | R-WUWC | Worker writes real on-spec code to working tree but `markTicketDone` is blocked (likely by `guardCompletionCommitBeforeDone` from F1-F3+F5 sweep); ticket wedges at Failed for `closer_handoff_terminal` 2/2 iterations; pickle then logs `Phase pickle completed successfully`; worker output is silently lost without operator salvage. **Fresh post-v1.75.5 reproducer of Finding #2 class.** Observed 2026-05-18 PM session `2026-05-18-6108815e/fb4b547f`: 357 LOC produced (`citation-spike.ts` 141 + `cost-latency-measure.ts` 216), 0 commits, 48m 45s wasted. Worker code recovered manually as `dbec6699d`. **Use this session as the regression fixture for B-PIPE-FIX R-PIPE-2/3/4 validation** | `prds/BUG-REPORT-2026-05-18-pipeline-launch-friction.md` Addendum Bug 5. **Likely covered by B-PIPE-FIX in flight**; filing as concrete observable + recovery procedure |
| 53 | R-SRAA | `[FATAL] refreshScope: archive already exists (refusing overwrite): scope.anatomy-park.json` crashes anatomy-park on every pipeline relaunch against a previously-run session. Operator must `rm -f $SESSION/archive/scope.*.json` manually between runs. Fix: rotate-and-overwrite (timestamp suffix on old archive) or detect prior-run `exit_reason: "fatal"` in state.json and proactively clean | `prds/BUG-REPORT-2026-05-18-pipeline-launch-friction.md` Addendum Bug 6 |
...
| 54 | R-MRFP | `MULTI-REPO DETECTED` warning false-positives on monorepos with workspace `working_dir:` per ticket (`packages/api`, `packages/app`, repo root). Detector computes "repos" from frontmatter without walking up to common `.git`. Cosmetic / log-noise; does not block. Fix: walk each `working_dir` to `.git` and dedupe before warning. Observed every relaunch of 2026-05-18 session `2026-05-18-6108815e` (3 of 3 launches) | `prds/BUG-REPORT-2026-05-18-pipeline-launch-friction.md` Addendum Bug 7 |
| 57 | R-RPRA | `check-readiness.ts` strips leading `/` from absolute paths and resolves them relative to `TARGET`. `source_prd` frontmatter pointing at session PRD (`~/.local/share/pickle-rick/sessions/<id>/prd.md`) fails for every ticket — 13 identical false-positive findings on a 15-ticket bundle. Also: backtick-wrapped prose paths (`(likely \`packages/api/src/.../foo.ts\`)`) are treated as hard file references. Operator-supplied refinement seeds for R-FRA / R-RTRC-7. Observed 2026-05-19 session `2026-05-19-65fd1881` (LOA-727/754/785 consolidated PRD) | `prds/BUG-REPORT-2026-05-19-readiness-absolute-path-outside-target.md`. Folds into R-FRA refinement (forward-create gate) and R-RTRC-7 (readiness exit/error semantics); no new PRD slot needed |
...
| **B-SSDF** | 🟡 FOLDED INTO MEGA | R-SSDF (Finding #46) | ~6 + hardening | Folded 2026-05-19 into `prds/p1-self-fix-mega-campaign-2026-05-19.md` (R-MEGA-SELF-FIX, Phase 2). Grok stand-in executing combined pipeline. |
...
| **B-PIPE-FIX** | 🟡 FOLDED INTO MEGA | R-PIPE-3 + R-PIPE-4 + T-HARDEN + C-PIPE-CLOSER (remaining) | 4 + closer | Folded 2026-05-19 into `prds/p1-self-fix-mega-campaign-2026-05-19.md` (R-MEGA-SELF-FIX). Grok stand-in mega-pipeline (4-5 serious bug PRDs combined). Phase 0 priority in the mega campaign. R-PIPE-1/2 already shipped. |
| **B-SJET-2** | 🟡 FOLDED INTO MEGA | R-SJET-3/4/6 + 4 T-HARDEN + C-SJET-CLOSER (Finding #47) | ~7 + closer | Folded 2026-05-19 into `prds/p1-self-fix-mega-campaign-2026-05-19.md` (R-MEGA-SELF-FIX). Grok stand-in executing the combined 5-PRD pipeline. See mega PRD for unified ticket list + Phase 1 priority. |
