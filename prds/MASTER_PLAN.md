...
**Last updated**: 2026-05-04 PM (slot 1l codex-spark wiring **SHIPPED locally** via agent team — `gpt-5.3-codex-spark` now reachable from codex spawns; deployed via install.sh; 113/113 affected-suite tests green; not pushed) (**Reliability bundle pickle-phase COMPLETE on session `2026-05-03-7d9ee8cc`, citadel/anatomy-park/szechuan-sauce phases NOT run** — 38/38 tickets Done (last ticket `dddee00b` completed manually on 2026-05-04 PM after worker silent-exit; 4 atomic audit commits `5218a33` / `ab413d7` / `9381e4d` / `bd81e2e`). Pipeline marked `step=completed exit_reason=failed`, 0/4 phases ran, `tool_retry_circuit_open` ×2. Forensic forward review found **cross-backend leak** (queue slot 1j NEW P1): 11+ worker logs across 8 ticket dirs ran codex despite `state.backend="claude"`. **DO NOT relaunch this session as-is** until 1j (cross-backend leak) and 1g (typescript symlink + auto-resume) ship. **50 unpushed commits**, 0 tagged. v1.69.0 ceremony from 2026-05-03 still in place. User decision 2026-05-04 PM: move on to bug fixes before any release ceremony. Codex usage resets May 5 12:31 AM.)
...
|---|---|---|---|
| 1 | [`prds/p1-reliability-and-test-coverage-bundle-2026-05-03.md`](p1-reliability-and-test-coverage-bundle-2026-05-03.md) ⭐ **P1 BUNDLE — PICKLE PHASE COMPLETE, DOWNSTREAM PHASES BLOCKED** | **38/38 tickets Done** on session `2026-05-03-7d9ee8cc`. Pipeline `step=completed exit_reason=failed`, **0/4 phases ran** (citadel/anatomy-park/szechuan-sauce never entered). Last ticket `dddee00b` (audit/cross-ref) completed manually 2026-05-04 PM after worker silent-exit (commits `5218a33`, `ab413d7`, `9381e4d`, `bd81e2e`). Bundle integrity is **suspect** — slot 1j cross-backend leak found 11+ worker logs ran codex despite `state.backend="claude"`. **DO NOT relaunch this session** until 1j + 1g ship. 50 unpushed commits ahead of origin. `bash install.sh` deployed locally 2026-05-04 PM (typescript-package symlink workaround in place; `pipeline-runner.js --help` exit 0). Pre-existing test flakes in `szechuan-sauce-gate-integration.test.js` + `scope-resolver-import-walks.test.js` (slot 1d territory) — not caused by audit work. | DEFERRED — fix 1j and 1g first, then either re-run citadel/anatomy/szechuan on this session OR start a fresh session. Decision: bug-fix bundle BEFORE any release/tag ceremony. |
...
| 1e | [`prds/p2-refined-tickets-trip-readiness-contract-resolver.md`](p2-refined-tickets-trip-readiness-contract-resolver.md) ⭐ **NEW P2** | **Draft** — readiness contract resolver fails on 5 false-positive classes: bundle-created forward refs, path-resolution misses, stdlib APIs, JSON output fields, test-defined helpers. 6 R-RTRC requirements; perf fix portion (cache + computeOneHop skip + RC-5 doc-extension allowlist) shipped 2026-05-03 PM (`701cd25`-era), reducing wall from 26m→72s. Remaining: refinement-team prompt update + (created by ticket) annotation + resolver allowlist + `tests/` inclusion + git-ls-files suffix fallback + regression fixture. | Bundle into next overnight after reliability bundle ships |
...
| 1g | [`prds/p1-deploy-typescript-symlink-and-cap-no-auto-resume.md`](p1-deploy-typescript-symlink-and-cap-no-auto-resume.md) ⭐ **NEW P1** | **Draft** — Two second-order bugs that surfaced once R-ICP-1..6 stopped silently papering over them: (A) `install.sh` doesn't symlink the typescript PACKAGE (only the `tsc` binary); pipeline-runner crashes ERR_MODULE_NOT_FOUND when collectTickets transitively pulls in citadel/frontend-prop-drift-audit. Workaround `ln -sf` confirmed (in place on this machine 2026-05-04 09:08Z); (B) per-ticket iteration cap halts pipeline with no auto-resume — 38-ticket bundles need 3-7 manual relaunches. 5 R-DTS + 5 R-CNAR requirements; recommend fix-A first (deploy gap), then bump per-tier caps + add auto-resume daemon. | Fix in next agent-team round; bundle-blocking |
| 1h | [`prds/p2-worker-silent-exit-and-ticket-path-drift.md`](p2-worker-silent-exit-and-ticket-path-drift.md) ⭐ **NEW P2** | **Draft** — Surfaced on ticket `ab62807f`: worker exited after research-review APPROVED with NO downstream lifecycle artifacts (no plan/implement/verify/review). `worker_session_<pid>.log` was 0 bytes (silent exit). Two RC: RC-1 worker session log 0-byte exits; RC-2 refinement produces tickets with non-existent paths. **Subsumed by 1i** (this PRD documents 2 of the 7 defect classes captured by the systemic bug). 4 R-WSE + 4 R-RPD requirements. | Reframe as scoped subset of 1i; or delete and migrate the R-WSE-* (worker silent-exit) requirements into 1i |
| 1i | [`prds/p1-ticket-authoring-quality-systemic-defects.md`](p1-ticket-authoring-quality-systemic-defects.md) ⭐ **NEW P1 — empirical 54% defect rate** | **Draft** — Read-only review of 13 remaining bundle tickets found 7 BROKEN, 5 WEAK, 1 PASS (92% defect rate). Seven recurring defect classes: file-path drift, self-referential ACs, missing dependencies, wrong-HEAD-state assumptions, cross-doc naming drift, hallucinated premises, literal value drift. RC-1: refinement-team analysts don't verify output against HEAD. RC-2: synthesis loses ground-truth fidelity. RC-3: no automated post-decomposition validator. RC-4: decomposition skill lacks failure-mode checklist. 7 R-TAQ requirements anchored on `audit-ticket-bundle.js` (mux-runner pre-flight) + analyst verification block + Failure-mode checklist in pickle-refine-prd.md. | Fix in next bundle; this bundle's broken tickets being repaired by parallel agents now |
| 1j | [`prds/p1-worker-spawns-codex-despite-claude-backend.md`](p1-worker-spawns-codex-despite-claude-backend.md) ⭐ **NEW P1 — cross-backend leak** | **Draft** — Forensic on session `2026-05-03-7d9ee8cc` after bundle finished 37/38: `state.backend="claude"` but **11+ worker logs across 8 ticket dirs** end with the codex CLI's signature output (`Reading additional input from stdin...` + `chatgpt.com/codex/settings/usage` usage-limit error). Workers spawned codex despite the backend flip — pipeline-status.json `failed`, 0/4 phases ran. Bundle-shipping integrity undermined: it is unclear which of the 22 unpushed commits were authored by claude vs codex workers running until they hit the usage limit. 8 R-XBL requirements: pre-spawn backend-resolution diagnostic + single-source-of-truth from `StateManager.read()` + spawn-time mismatch assertion + `audit-worker-backends.ts` backfill script + integration test + trap-door invariant. Sub-bug of `1h` recurrence (same `dddee00b` failure) but with completely different root cause and fix surface. | Fix BEFORE re-running any partial-bundle session; bundle-blocking, integrity-critical |
| 1l | **P2 — codex backend `gpt-5.3-codex-spark` wiring** | **SHIPPED locally 2026-05-04 PM** — agent team landed: `pickle_settings.default_codex_model="gpt-5.3-codex-spark"`, new `State.codex_model?: string`, exported `resolveCodexModel(extensionRoot, state)` helper in spawn-morty.ts (precedence: state → settings → undefined), threaded through `mux-runner.ts:1232` manager invocation + `microverse-runner.ts` worker spawn. Refinement-team intentionally skipped — `REFINEMENT_BACKEND` is hardcoded to `'claude'` (planning, not implementation; codex branch unreachable by design). Tests: 1 backend-spawn + 6 spawn-morty precedence tests added (113/113 green across affected suites + trap-door conformance). Trap-door entry + state.json field invariant added to `extension/CLAUDE.md`. Compiled JS deployed via `bash install.sh` 2026-05-04 PM (md5 parity confirmed). NOT pushed per user direction. Codex spawns from pickle-rick now use spark by default — much bigger budget for the next bug-fix bundle. | Done; tag with next release |
...
## 3. Current State (verified 2026-05-04 AM)

| Item | Value |
|---|---|
| Source version | **v1.69.0** (commit `bdc775f`) + 5 unpushed local commits ahead with R-ICP-1..6 + perf fix + 3 bug PRDs |
| Deployed version | **v1.69.0** + locally-deployed R-ICP fixes (md5 parity confirmed for mux-runner.js, pipeline-runner.js, check-readiness.js) + manual typescript symlink workaround applied |
| Latest release on GitHub | **v1.69.0** (no new tag; the R-ICP fixes are local-only, not yet shipped to GitHub) |
| Branch state | `main`, **5 local commits ahead of `origin/main`** — NOT pushed per user instruction. Push-when-ready set: `434e33d`, `a7ed2a9`, `2de4c24`, `1a7c0f2`, [pending p2-worker-silent-exit commit] |
| Working tree | CLEAN modulo bundle/ac-dr-02.json test noise |
| Active pipeline session | **`2026-05-03-7d9ee8cc` IN FLIGHT on claude backend** — 25/38 tickets Done, current_ticket=`ab62807f` (Failed; pipeline moves to next), iteration 4/15 of current batch. Codex usage limit until May 5 12:31 AM. Pipeline cycles ~20-25 min wall per iteration on claude. |
