...
**Last updated**: 2026-05-04 evening (P1 bug-fix bundle 2026-05-04 PAUSED on session `2026-05-04-f416c6cc` after 28-min run #2. **R-XBL-1 / -2 / -2b / -3 + R-CNAR-1 SHIPPED** (commits `7793a11 9437b0c 817e73c a3641e3 616f474 95f2c37`); state.backend leak fully diagnosed and fixes deployed. Run #2 tripped circuit-breaker (no-progress, tier=small, budget=4) because (a) **codex-spark MANAGER hallucinated** "I'll try one last time under Hermes" and wrote `state.backend='hermes'` mid-iteration — the canonical write-side leak that R-XBL-3 now catches pre-spawn; (b) **codex-spark WORKERS skip the `completion_commit:` frontmatter contract** so phantom-Done watcher correctly reverted 3 truly-done tickets to Todo, fueling the no-progress trip; (c) **install.sh did not deploy `extension/types/index.js`** on first run, so the running pipeline rejected `worker_spawn_backend_resolved` events as unknown and lost activity-log fidelity (re-running install.sh fixed parity). R12 (codex-spark unproven manager reliability) materialized — recommend split: **claude/sonnet manager + codex-spark workers** (slot 1o). **80 unpushed commits**, 0 tagged. v1.66.0 still GitHub-Latest with poison content; bundle closer ships v1.70.0. Slot 1l codex-spark wiring SHIPPED locally; tagged with v1.70.0.)
...
## 🟢 Just shipped (2026-05-04 PM/evening)
...
Bundle is **paused** on `2026-05-04-f416c6cc` after run #2 circuit-tripped. Run #3 prep:

1. **Restore phantom-reverted Done tickets.** `8224fc7f` (R-XBL-2), `160e8816` (R-XBL-2b), `4d7c4cfa` (R-XBL-3) all have real commits in git (`a3641e3 616f474 95f2c37`) but were reverted to Todo because the codex-spark worker didn't write `completion_commit:`. Patch frontmatter manually with the SHAs before relaunch.
2. **Ship slot 1o (manager/worker backend split, option B)** before relaunch — adds optional `state.worker_backend` field (~15 LOC + test); spawn-morty consults it before falling back to `state.backend`. Manager runs claude/sonnet (reliable orchestration); workers run codex-spark (parallel + cheap). Removes the F1 hallucination class entirely.
3. **Relaunch with `state.backend='claude'`, `state.worker_backend='codex'`, `state.codex_model='gpt-5.3-codex-spark'`.** R-XBL-3 deployed = catches any residual flip pre-spawn. Operator presence first 30min still applies (R-CNAR-6 smoke gate).
4. **After closer ships v1.70.0**: file the next bundle from the residual queue:
   - 1d test-flakes (council-publish + scope-resolver)
   - P3 dirty-tree-guard (R-PDT-1..4) + P2 stop-hook (R-SHB-1..4) from today's surfaced bugs
   - 1k monitor watchdog (residuals)
   - 1p (codex-spark worker frontmatter) + 1q (install.sh types deploy gap)
...
|---|---|---|---|
| **0** | [`prds/p1-bug-fix-bundle-2026-05-04.md`](p1-bug-fix-bundle-2026-05-04.md) ⭐ **P1 BUG-FIX BUNDLE — PAUSED 2026-05-04 evening** | **62 atomic tickets** (56 impl + 1 wire + 4 harden + 1 closer) on session `2026-05-04-f416c6cc`. Section A keystones SHIPPED (R-XBL-1/-2/-2b/-3 + R-CNAR-1, 5 commits). Run #2 (16:59→17:28 local) circuit-tripped after codex-spark MANAGER hallucinated `state.backend='hermes'` mid-iteration (the canonical write-side leak; R-XBL-3 now catches it pre-spawn). Workers also skipped `completion_commit:` frontmatter, so phantom-Done watcher reverted truly-done tickets to Todo, fueling no-progress trip. install.sh first run failed to deploy `types/index.js` (md5 mismatch); re-running fixed parity. **R12 (codex-spark unproven manager) materialized.** Bootstrap flags applied: `state.flags.bundle_bootstrap_mode="2026-05-04-v1.70.0"` + `state.flags.skip_readiness_reason`. Cycle-3 critical refinements baked in. | (1) Restore commits-have-tickets parity for `8224fc7f / 160e8816 / 4d7c4cfa` (manual `completion_commit:` patch); (2) ship slot 1o (worker_backend split); (3) relaunch with claude manager + codex-spark workers + R-XBL-3 deployed. |
...
| 1g | [`prds/p1-deploy-typescript-symlink-and-cap-no-auto-resume.md`](p1-deploy-typescript-symlink-and-cap-no-auto-resume.md) **NEW P1** | **Composed into bundle 2026-05-04 Section B**. R-DTS-1 marked REGRESSION-TEST-ONLY (already-shipped at install.sh:305-310 per Cycle-3 codebase analyst). R-DTS-2/-3 + R-CNAR-1..6 (incl. NEW R-CNAR-6 spark smoke gate) IMPLEMENT. Bundle drops `xlarge` tier (5-invariant blast radius); raises `large=60`. AC-DTS-02 replaced (nonexistent --help → module-load smoke test). | Land via bundle slot #0 |
