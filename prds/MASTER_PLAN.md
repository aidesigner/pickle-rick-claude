...
| Active pipeline | **B-DWF-2** (row 13, P3) — `prds/p3-bug-fix-bundle-b-dwf2-retire-refinement-subprocess.md`, **LAUNCHING 2026-06-01**. Retires the legacy refinement subprocess after a hard-gate live soak (R-DWF-3-SOAK first; FAIL → shelve, no retirement). The soak is encoded as the bundle's own blocker ticket (no operator gate needed) + the release gate backstops the fallback-removal. **11 releases shipped this session (v1.89.0–v1.92.0); 2 stale findings closed.** Only other residue: #25 R-CSI (external-event-gated forensics). |
...
| 13 | **B-DWF-2** (LAUNCHING) | P3 | (feature — R-DWF-3/6) | `prds/p3-bug-fix-bundle-b-dwf2-retire-refinement-subprocess.md` (authored 2026-06-01). Retire `spawn-refinement-team.ts` + `refinement-watcher.ts` + skill tmux block + `PICKLE_REFINE_WORKFLOW` kill-switch. **Soak is encoded as a hard-gate first ticket (R-DWF-3-SOAK)** — live real-agent run of the v1.91.0 workflow path on a real PRD; FAIL → PRD `status: Shelved`, no retirement (legacy path retained). Then R-DWF-3 (retire) + R-DWF-6 (docs). MINOR (→1.93.0). | ~3 |
