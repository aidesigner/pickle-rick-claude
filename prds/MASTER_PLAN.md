...
| 74 | R-WSWA | B-WEDGE session `pickle-e4f1269f` (2026-05-25) killed by R-WSRC-2 graceful exit `state_schema_version_ahead` after 30 min of work. Some worker wrote a forward-schema state.json.tmp.* (LATEST_SCHEMA_VERSION=4 but tmp had higher value). Runtime correctly aborted per the R-QGSK-3 wedge prevention contract, BUT the bundle lost all in-flight ticket work + no operator-visible WHO/WHY signal — only the terminal exit_reason. Need: forensic event `state_write_schema_version_violation` (already exists in VALID_ACTIVITY_EVENTS!) must include `{caller_pid, caller_stack_hint, attempted_schema_version}` so the operator can attribute the violation to a specific worker subprocess. Sized: ~3 tickets. |
