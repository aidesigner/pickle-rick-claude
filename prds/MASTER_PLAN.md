...
| Active pipeline | **B-LSOF** (drain row 10, P3, #37e R-PIWG-5) — `prds/p2-bug-fix-bundle-b-lsof-launch-git-probe.md`, running 2026-06-01 (1/4: R-PIWG-5.1 done, R-PIWG-5.2 in conformance). Advisory launch-time concurrent-git-access probe (extract fail-OPEN helper + setup.ts wire-in + event). Schema-neutral (PATCH → v1.89.7). 7 bundles shipped this session (1.89.0–1.89.6); B-MONITOR closed as already-shipped. Next: R-PSAI + B-LASP (P3 bugs), then feature epics (B-DWF/B-DSEK). |
...
| 10 | **B-LSOF** | P3 | #37e R-PIWG-5 | `prds/p2-bug-fix-bundle-b-lsof-launch-git-probe.md` (extract fail-OPEN `probeConcurrentGitAccess` from cancel.ts lsof/pgrep pattern + advisory launch-time wire-in + `concurrent_git_access_detected` event; schema-neutral, advisory-only). **LAUNCHING 2026-06-01.** | ~4 |
| 11 | **R-PSAI** | P3 | #12 | `prds/p2-pickle-pipeline-no-scope-auto-inference.md` (UX friction; lowest) | ~2 |
| 11b | **B-LASP** | P3 | #87 R-LASP | Author from finding #87 (no PRD yet): deployed `bin/log-activity.js:36` resolves the activity schema via `src/types/…` — a path `install.sh` never creates → `Failed to load activity schema: ENOENT`, fail-open (validation silently skipped on every activity log). Fix: align the TS-source schema resolution with `install.sh`'s deploy target (`extension/` root + a `src/types/` fallback for the in-repo case) + a deploy-parity test. Schema-neutral, P3 tooling. **Bug → drains before the feature epics.** | ~2 |
