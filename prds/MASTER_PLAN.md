...
| 76 | R-WSRC-GR-LEAK | **R-WSRC-GR-1 hook gap surfaced 2026-05-25 08:08Z** on B-PNTR session `pickle-f28e140f`: worker for ticket `373c9deb` (R-PNTR-1) had its commits dropped TWICE by worker-level `git reset` (lost commits `0ac32955` then `462cd9ef`); operator restored via `git restore --source 462cd9ef --staged` as `d586b545`. The `config-protection.ts` hook shipped in B-WSRC-GR v1.79.2 (commit `1dd1e8da`, R-WSRC-GR-1) DID NOT BLOCK these resets. Hypothesis: PICKLE_ROLE env discrimination misses some worker spawn paths (codex-rescue subagent? send-to-morty wrapper?). 6th R-WSRC-GR violation today despite the hook being live. Sized: ~2 tickets (audit all worker spawn paths for PICKLE_ROLE, regression test fixture for non-direct-spawn). |
