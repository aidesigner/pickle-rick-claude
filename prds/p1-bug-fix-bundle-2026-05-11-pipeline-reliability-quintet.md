...
- [ ] **AC-BUNDLE-02** — All five new activity events (`manager_max_turns_relaunch`, `signal_received`, `worker_gate_failed`, `cross_ticket_regression_detected`, `recoverable_phase_failure`, `subprocess_error`) registered in `VALID_ACTIVITY_EVENTS`. Test asserts `activityEventRegistry` count equals `VALID_ACTIVITY_EVENTS.length` (per Class A lesson from bundle 2026-05-10 — drive count from `Object.keys`, NOT a hardcoded number).
- [ ] **AC-BUNDLE-03** — All five new trap-door entries pinned in `extension/CLAUDE.md`. `bash extension/scripts/audit-trap-door-enforcement.sh` exits 0 with the new ENFORCE references.
...
- [ ] **AC-BUNDLE-08** — End-to-end fixture pipeline: 2-phase run (pickle + citadel) where pickle exits 1 with ≥1 commit → citadel still runs → final `state.json` has `recoverable_phase_failure` activity event AND `signal_received` event (when SIGTERM delivered to a stage subprocess).
- [ ] **AC-BUNDLE-09** — Full release gate green at closer (per R-CLOSER-2). Caveat: any residuals from the inherited 47 test:fast failures from bundle 2026-05-10 are handled by the **subsequent remediation bundle** (`prds/p1-bug-fix-bundle-2026-05-11-remediation-53-failures.md`), NOT this bundle. Bundle's closer logic auto-detects whether the 47 residuals are still present and emits a warning if so.
