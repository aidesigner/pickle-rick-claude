...
The bundle introduces or renames **6 activity events** (full set, exhaustive): `manager_max_turns_relaunch` (R-MMTR-4, NEW), `signal_received` (R-SOA-1, NEW), `worker_gate_failed` (R-PTG-5, NEW; companion renames `worker_lint_gate_passed`→`worker_gate_passed` and `worker_lint_autofix_applied`→`worker_gate_autofix_applied` MAY be deferred to a follow-up bundle), `cross_ticket_regression_detected` (R-PTG-6, NEW), `recoverable_phase_failure` (R-PHC-3, NEW), `subprocess_error` (R-APMW-5, NEW). These six are the canonical set for AC-BUNDLE-02 / AC-BUNDLE-08 verification.
...
- [ ] **AC-BUNDLE-08** — For every event E in the canonical 6-event set, the bundle's E2E fixture pipeline exercises E's triggering condition and asserts E appears in `state.json.activity` with all required payload fields per E's schema definition. One fixture per event (6 fixtures total); fixtures live in `extension/tests/integration/bundle-2026-05-11-events/`.
