...
- **AC-CCPM-5.b** — All 3 new activity events (`codex_manager_self_bootstrap_attempted` (created by R-CCPM-5), `orphan_session_detected` (created by R-CCPM-5), `state_worker_timeout_drift_corrected` (created by R-CCPM-5)) registered in `VALID_ACTIVITY_EVENTS` + `activity-events.schema.json` `oneOf` + `EVENT_CASES` + `EVENT_NAMES` + `spawn-refinement-team.ts:ACTIVITY_EVENT_SCHEMA_SECTION` (full triangle).
