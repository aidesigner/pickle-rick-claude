...
2. Graph-level attributes (alphabetical by key):
   - acceptance_criteria, cleanup (workspace only), default_max_retry, goal, label,
     model_stylesheet, repo_branch (workspace only), repo_url (workspace only),
     retry_target, spec_file, workspace (workspace only), working_dir
3. Blank line
