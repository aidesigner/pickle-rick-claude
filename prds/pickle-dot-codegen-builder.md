...
Today, `/pickle-dot` converts PRDs to attractor-compatible DOT pipelines using ~870 lines of prompt engineering across two markdown files. The entire 30-pattern system — including structural rules, anti-patterns, validation checks, and defense-in-depth layers — must be internalized by the LLM on every invocation. There is no programmatic enforcement, no type safety, and no automated validation before submission. *(refined: pattern count corrected to 30 pipeline composition patterns — excludes 3 superseded (5, 7, 12) and 5 defensive coding patterns (26-30))*
...
**Current Process**: `/pickle-dot` is a 440-line command prompt + 555-line patterns reference. The LLM must correctly apply 30 patterns, avoid 59 anti-patterns, perform 4 pre-submission checks, and emit syntactically valid DOT — all from prompt context alone. No unit tests exist for DOT generation correctness. *(refined: pattern count corrected to 30 pipeline patterns; anti-pattern count corrected to 59; line counts updated)*
...
**Importance**: Each pattern omission or schema error costs 1-4 hours of failed pipeline execution. The system handles 30 patterns × 59 anti-patterns × handler-specific rules — this complexity exceeds what prompt engineering alone can reliably manage.
...
3. Builder generates DOT with:
   ...
   - Security scan subgraph after auth impl node
   ...
   - Defense matrix: competitive=false, specDriven="spec_file + conformance", adversarial=false
...
| P0 | Builder enforces single start node (Mdiamond) and single exit node (Msquare) | As a pipeline author, structural invariants must hold | `.build()` throws `BuildError` if start/exit count ≠ 1 |
| P0 | Builder enforces acceptance_criteria ↔ context_on_success mapping completeness | As a pipeline author, I need exit convergence guaranteed | `.build()` throws `BuildError` with code `MISSING_AC_MAPPING` listing unmapped keys |
...
| P0 | Builder performs internal structural validation (15 rules) on `.build()` output | As a pipeline author, structural errors must be caught even without attractor CLI | `.build()` returns `BuildResult` with validation diagnostics *(refined: elevated from fallback to primary; expanded to 15 rules)* |
...
| P0 | Fix loop tracks error count per iteration; reverts to best prior attempt if errors increase | As a pipeline author, fix iterations must converge | Unit test: error count [5, 3, 4] → reverts to attempt 2 (count 3) *(refined: new)* |
...
| P1 | Builder enforces `allowed_paths` on all codergen impl nodes | As a pipeline author, scope enforcement must be complete | `.build()` throws `BuildError` with code `MISSING_ALLOWED_PATHS` if any codergen node lacks `allowed_paths` *(microverse: iter3 #11 — was "warns", contradicted rule 15 and BuildErrorCode)* |
...
**Acceptance criterion**: `BuildResult.patternsApplied` includes every `auto` and `default-on` pattern for any non-trivial pipeline (≥1 phase). Opt-in patterns appear only when explicitly invoked.
...
| (verify_final) | `verify_final` | After review ratchet (or after last phase's conformance if no ratchet), before `exit` | `shape="box"`, `class="codergen"`, `timeout="30m"`, `tool_command="cd \${WORKING_DIR} && npx tsc --noEmit && npx eslint src/ --max-warnings=-1 && npm test"`, `retry_target="fix_all"`, `max_visits=3`. Sets `context_on_success` with Tier 2 auto-generated keys (`types_compile=true`, `lint_clean=true`, `tests_pass=true`) plus any Tier 1 explicit keys not already set by phase verify nodes. Attributes (`allowed_paths`, `escalate_on`, `permission_mode`) per cross-phase inheritance rule. *(microverse: iter3 #6)* |
| 25 (Catastrophic Recovery) | N/A (edge attribute) | On `verify_final` → `setup_deps` edge | `loop_restart=true` on edge; emitted when any node in the pipeline has ≥1 incoming edge from a diamond `outcome=fail` branch or is referenced by a `retry_target` attribute (i.e., the pipeline contains retry loops). NOT emitted for zero-phase pipelines or pipelines with only forward edges. *(review: replaced qualitative "max_visits exhaustion risk" with binary predicate)* |
...
| 20 (Microverse) | `optimize_${name}`, `measure_${name}`, `compare_${name}`, `check_${name}` | Self-contained subgraph after designated phase | `optimize [class="codergen"]` → `measure [tool_command="${measureCommand}"]` → `compare [class="review", read_only=true]` → `check [shape="diamond"]` — outcome=improved loops to optimize, outcome=converged proceeds. `max_visits` from `MicroverseOpts.maxVisits`. |

**Pattern 0e (Progress Gate) prerequisite**: Progress gate uses `git status --porcelain`. Builder emits this node only for pipelines running in a git-initialized workspace. Non-git workspaces skip progress gates (emit info diagnostic: `"Progress gate skipped: not a git repository"`).
...
interface BuildResult {
  ...
  slug: string;
  patternsApplied: string[];    // e.g., ["0a", "0b", "0c", "0d", "0e", "1", "3", "4", "6", "6b", "10", "13", "14", "15", "21", "22", "23", "25"] (auto patterns only — opt-in patterns like "16b" appear when explicitly requested)
...
interface DefenseMatrix {
  ...
  specDriven: string;         // "spec_file + BDD + conformance" | "spec_file + conformance" | "NONE"
...
2. Graph-level attributes (alphabetical by key):
   - acceptance_criteria, default_max_retry, goal, label, model_stylesheet,
     retry_target, spec_file, working_dir
3. Blank line
...
6. Nodes in pipeline-order (topological sort, ties broken by insertion order):
   ...
   d2. Per-phase opt-in nodes (in phase declaration order): red_team_${phase} (Pattern 17, emitted after phase's conformance node)
   e. verify_final
   f. exit node (Msquare)
7. Blank line
...
| CUJ-5 | `dot-builder-patterns.test.js` | 3-phase mixed patterns (auth+security, api+goalgate, docs) | Correct thread_ids, security scan subgraph, goal gate with retry, test dirs auto-added, all auto patterns applied *(refined: new)* |

### Edge Cases

| Condition | Behavior | Test |
|:---|:---|:---|
| Empty pipeline (no phases) | `.build()` produces valid `start → setup_deps → capture_baseline → verify_final → exit` (auto infrastructure always emitted; no per-phase nodes). `verify_final` sets Tier 2 AC keys. Pattern 25 (catastrophic recovery) is NOT emitted (no retry loops exist). `patternsApplied` includes only: `0a, 0b, 0c, 21, 23`. *(review: specified zero-phase topology explicitly)* | Unit test |
...
| Fan-out with 1 branch (degenerate) | Throws: component fan-out must have ≥2 branches | Unit test *(council: adversarial)* |
...
| 65 | a1b3c5d7 | Fallback schema creation | Medium | CLI working | `types/attractor-schema.fallback.ts` committed with minimum viable attributes; builder loads it when `$ATTRACTOR_ROOT` unavailable | `types/attractor-schema.fallback.ts` | `extension/src/types/**`, `extension/tests/**` | No |
...
| 40 | Auto patterns | All `auto` mode patterns from the Pattern Application Matrix are applied by `.build()` without user invocation: 0a (setup_deps), 0b (max_parallel), 0c (baseline), 0d (delta-aware), 0e (progress gate), 1 (test-fix), 3 (conditional routing), 4 (fan-out when ≥2 phases), 6 (max_visits), 6b (read_only+STATUS), 10 (scope creep), 13 (lint), 14 (typecheck), 15 (conformance), 16b (BDD when 3+ reqs), 21 (fix_all), 22 (permission scoping), 23 (defense matrix), 25 (catastrophic recovery). | `BuildResult.patternsApplied` includes every auto pattern for a 2-phase spec with 3+ requirements |
| 50 | Opt-in patterns | Each opt-in pattern has: (a) a PhaseSpec/builder method that activates it, (b) a snapshot test proving the DOT output, (c) a test proving it's NOT applied when not requested. Patterns: 0 (workspace), 2 (goal gates), 8 (security scan), 9 (coverage), 16 (spec-first — default-on, verify opt-out works), 17 (red team), 18 (competing impls), 19 (review ratchet), 20 (microverse). | Verify `competing: true` activates Pattern 18 (component→tripleoctagon), NOT Pattern 4 (independent phases) |
...
| 65 | Fallback schema | `types/attractor-schema.fallback.ts` is committed to git with hardcoded attributes. Contains minimum viable set: `goal`, `goal_gate`, `retry_target`, `max_visits`, `thread_id`, `timeout`, `allowed_paths`, `read_only`, `context_on_success`, `working_dir`, `default_max_retry`, `label`, `acceptance_criteria`, `model_stylesheet`. Builder loads fallback when `$ATTRACTOR_ROOT` is unavailable. | Fallback covers all attributes referenced by the 15 validation rules *(council: H2)* |
...
| BDD Scenarios (16b) | **Phases 20, 30, 40** — these have 3+ requirements each with Given/When/Then potential |
