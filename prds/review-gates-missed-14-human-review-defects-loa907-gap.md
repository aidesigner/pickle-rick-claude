...
- **Dead guard / no-op flag / pre-migration behavior parity** (#15) — three concrete, checkable shapes (this is the *operationalized* form of the `fix-set-regressions` / `observability-correctness` lenses the ultracode review only *named*):
  1. **Unsatisfiable / tautological predicate** — a boolean guard introduced by the diff whose condition is always-true or always-false given the in-scope constants/types it references. **Resolve the constants and evaluate the inequality** (`firstPassCeiling(form) + ATTOM_SECOND_PASS_CEILING_MS >= APPRAISAL_EXTRACTION_LOCK_DURATION_MS` → always true). A guard that can never route to one of its branches → the gated branch is dead and the feature it gates is silently on/off forever.
...
- **AC-5 — Mechanism 2 report-only lens.** Add the skeptic dimension (including the dead-guard / no-op-flag / behavior-parity shapes) as prompt bullets on the existing citadel/anatomy analyzer; output goes to the report / PR comment (Council-directive style). It is **never** fed to the mechanical remediator and **never** used as a convergence signal. Surfaces #1, #2, #3, #7, #11, #13, #14, **#15** for a human/follow-up agent.
- **AC-6 — Fixture regression + no-wedge proof.** Reconstruct PR #1707's diff as a fixture; assert the invoked + extended analyzers surface **≥ #2, #3, #6, #7, #9, #10, #14**, and that the M2 lens flags **#15** — the always-true `attomSecondPassExceedsLockBudget` guard and the resulting no-op `ENABLE_ATTOM_ENRICHMENT` (the tautological-predicate shape is constant-resolvable, so it is a deterministic fixture assertion even though its mechanism is M2). Prove **G2** (clean diff → zero new findings) and **G1** (a finding-bearing run does not change the pipeline's terminal exit behavior vs today).
