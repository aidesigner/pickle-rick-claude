...
- **AC-2 — Coverage audit + extend the closest analyzer.** Run the existing analyzers against the LOA-907 diff fixture; for each gap, extend the nearest analyzer (`banned-casts`→`as never` #9; `sibling-auth`→missing budget/feature-flag/CSRF guard, not just auth; `stale-reference`→diff-introduced comments #14). **No parallel analyzers** — extend, don't duplicate.
...
- **AC-4 — Cheapest wins go to eslint, not the gate.** `curly` (brace-free one-liner `if`), `no-nested-ternary`, and a no-unnecessary-cast rule into the target's eslint config — caught at author time, zero gate cost. Recurred across 5+ corpus PRs.
