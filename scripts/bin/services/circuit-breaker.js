...
function transition(cbState, to, reason) {
...
export function checkProgress(current, cbState) {
    ...
    if (currentHead && currentHead !== cbState.last_known_head)
        return true;
    ...
    if (current.step !== cbState.last_known_step)
        return true;
    // Signal 5: Current ticket changed
    if (current.current_ticket !== cbState.last_known_ticket)
        return true;
    return false;
}
export function validateCBConfig(config) {
...
export function loadCBState(sessionDir) {
    const cbPath = path.join(sessionDir, 'circuit_breaker.json');
    const raw = readJsonSafe(cbPath, {});
    if (!raw.state || !['CLOSED', 'HALF_OPEN', 'OPEN'].includes(raw.state)) {
        return freshState();
    }
    return {
        state: raw.state,
...
export function saveCBState(sessionDir, cbState) {
    const cbPath = path.join(sessionDir, 'circuit_breaker.json');
    atomicWriteJson(cbPath, cbState);
}
export function recordIteration(sessionDir, state, error) {
    const configPath = path.join(sessionDir, '..', 'config.json');
    const rawConfig = readJsonSafe(configPath, {});
    const config = { ...DEFAULT_CONFIG };
    if (typeof rawConfig.cb_enabled === 'boolean')
        config.enabled = rawConfig.cb_enabled;
    if (typeof rawConfig.cb_no_progress_threshold === 'number')
        config.noProgressThreshold = rawConfig.cb_no_progress_threshold;
    if (typeof rawConfig.cb_same_error_threshold === 'number')
        config.sameErrorThreshold = rawConfig.cb_same_error_threshold;
    if (typeof rawConfig.cb_half_open_after === 'number')
        config.halfOpenAfter = rawConfig.cb_half_open_after;
    if (!config.enabled) {
    ...
    if (currentHead)
        cbState.last_known_head = currentHead;
