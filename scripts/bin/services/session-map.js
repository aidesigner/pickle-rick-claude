...
export async function withSessionMapLock(fn) {
...
// ---------------------------------------------------------------------------
export async function updateSessionMap(cwd, sessionPath) {
...
export async function removeFromSessionMap(cwd) {
    await withSessionMapLock(async () => {
        const mapPath = getSessionsMapPath();
        const map = readJsonSafe(mapPath, {});
        delete map[cwd];
...
export async function getSessionForCwd(cwd) {
    const mapPath = getSessionsMapPath();
    const map = readJsonSafe(mapPath, {});
...
// ---------------------------------------------------------------------------
export async function findLastSessionForCwd(targetCwd) {
    const sessionsRoot = getSessionsRoot();
    if (!fs.existsSync(sessionsRoot))
        return null;
    let bestDir = null;
    ...
    for (const entry of fs.readdirSync(sessionsRoot)) {
        ...
        try {
            const state = JSON.parse(fs.readFileSync(statePath, 'utf-8'));
            if (state.working_dir !== targetCwd)
                continue;
