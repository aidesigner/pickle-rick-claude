...
function buildHandoffSummary(state, sessionDir, iteration) {
    const lines = [
 ...
 */
export function classifyCompletion(output, state) {
    if (hasToken(output, PromiseTokens.EPIC_COMPLETED))
        return 'task_completed';
    if (hasToken(output, PromiseTokens.EXISTENCE_IS_PAIN))
        return 'review_clean';
    if (hasToken(output, PromiseTokens.THE_CITADEL_APPROVES))
        return 'review_clean';
    if (hasToken(output, PromiseTokens.TASK_COMPLETED))
        return 'task_completed';
    if (hasToken(output, PromiseTokens.PRD_COMPLETE))
        return 'continue';
    if (hasToken(output, PromiseTokens.TICKET_SELECTED))
        return 'continue';
 ...
 */
export function classifyIterationExit(exitCode, stdout, stderr, timedOut) {
 ...
 */
export function validateCommandTemplate(template) {
    if (/[/\\]|\.\./.test(template)) {
 ...
 */
export function transitionToMeeseeks(state) {
