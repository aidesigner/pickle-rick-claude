...
2. **Phase dispatch preflight**: Read `${EXTENSION_ROOT}/extension/data/phase-personas.json` with the Read tool before the first phase `Agent` call for this ticket. Assert `version >= 1`; mismatch is a hard failure. Resolve this ordered phase list from the JSON keys: `research`, `plan`, `implement`, `verify`, `review`, `refactor`.
...
3. **Spawn**: make six distinct sequential `Agent` calls, one per phase from `phase-personas.json`:
